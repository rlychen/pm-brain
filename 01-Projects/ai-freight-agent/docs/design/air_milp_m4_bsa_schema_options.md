...
**Status:** DECIDED (Session 27, 2026-06-04) — **Opt 1 (contract entity).** Builds both per_flight + equalized settlements in M4 (M4b folded in). User chose Opt 1 over the drafter's Opt-3-now lean (§4) to keep the equalized take-or-pay number available for the pitch and give the future Layer-3 `capacity_manager` a clean injection point.
