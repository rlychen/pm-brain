...
11. **Per-shipment subgraph pre-filter.** For each HAWB prune arcs failing its
    predicates (`cargo_type_ok`, embargo, lithium, screening, service-product
    mode/carrier, deadline reachability) → `A_k`. `A_k = ∅` → infeasible (structured
    rescue event; do not enter the MILP).
