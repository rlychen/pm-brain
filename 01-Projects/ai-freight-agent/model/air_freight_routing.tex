...
\begin{itemize}[noitemsep]
  \item the nine pre-filter predicates (\S\ref{sec:prefilter});
...
\midrule
Volume & $v_k$ & Cargo volume (sum over all pieces) (code \texttt{Hawb.volume\_cbm}) & m\textsuperscript{3} \\
Actual weight & $w_k$ & Gross weight (code \texttt{Hawb.weight\_kg}) & kg \\
Chargeable weight (per-HAWB billing reference, not used in bucket capacity) & $cw_k$ & $\max(w_k,\, v_k \cdot 167)$ (code: derived by \texttt{\_chargeable\_weight(hawb)}; not stored) & kg \\
Piece dimensions & $\text{pieces}(k)$ & Max single-piece $L \times W \times H$ for ULD-fit pre-filter (\emph{not in air MVP code} --- ingestion / graph-gen) & cm \\
Cargo-ready window (early) & $t_k^{\text{rdy,early}}$ & Earliest moment cargo is available at the origin door (code \texttt{Hawb.ready\_early\_h}) & h (UTC) \\
Cargo-ready window (late) & $t_k^{\text{rdy,late}}$ & Latest moment within the shipper's window (code \texttt{Hawb.ready\_late\_h}) & h (UTC) \\
Per-HAWB origin-side dispatch lead time & $\lambda^{\text{disp}}_k$ & Backward-binding offset: the truck must depart the origin door no later than $\text{CO}_a^* - \lambda^{\text{disp}}_k$ to make the cutoff $\text{CO}_a^*$ of any candidate outbound air arc $a$. Accumulates: pickup-to-CFS road transit + origin-CFS receiving + ULD build + tender-to-ramp lead time + queue contingency. Tenant-calibrated lookup keyed on (origin region, cargo type, MAWB consolidation profile); typical 4--8h on mid-sized origin lanes, longer for DGR / VAL builds. Used in the forward-time-window propagation's pickup-feasibility check (\S\ref{sec:fwd-time-propagation}). (code \texttt{Hawb.lambda\_disp\_h}) & h \\
Latest valid arrival time (``hard backstop'') & $T_k^{\text{abs}}$ & \textbf{Mandatory-finite for every HAWB.} Semantically the cargo-death point for PER / AVI / HUM / VAL (spoilage, expiration, security window) and the customer-cancellation point for GEN (commercial value lost; shipment refunded). \emph{Not} a hard MILP constraint --- it is the arrival time of the per-HAWB fallback arc (\S\ref{sec:fallback-arc}) and bounds the worst-case quadratic tardiness penalty (C.10). For GEN, ingestion applies a tenant-configured default (e.g.\ $t_k^{\text{rdy,early}} + 30$ d) when the shipper does not specify. \textbf{Pre-MILP guard:} ingestion asserts $T_k^{\text{abs}}$ is finite and $\geq \Delta_k$; failure surfaces as a structured intake error, not silent corruption of the C.14 $\tau_k$ bound or the fallback $\text{arr\_dest}$ scalar. (code \texttt{Hawb.deadline\_abs\_h}) & h (UTC) \\
Service-product soft deadline & $T_k^{\text{SLA}}$ & $= t_k^{\text{rdy,early}} + T^{\text{SLA}}_{\text{sp}(k)}$ (\S\ref{sec:service-products}) (code: computed by \texttt{flexibility.committed\_deadline}; D-F6 v2 SLA, not a stored field) & h (UTC) \\
Shipper-contractual deadline & $T_k^{\text{dead}}$ & Optional shipper-contracted door-to-door deadline; absent when the service product alone governs. Used only as one of the two terms in $\Delta_k$ (next row). (code \texttt{HawbArrival.t\_dead\_at}, set by the generator) & h (UTC) \\
Effective soft deadline & $\Delta_k$ & $\min(T_k^{\text{dead}},\, T_k^{\text{SLA}})$ if $T_k^{\text{dead}}$ exists; otherwise $\Delta_k = T_k^{\text{SLA}}$. Used in C.10. (code \texttt{Hawb.soft\_deadline\_h}) & h (UTC) \\
Declared shipment value & $\text{value}_k$ & Shipper-declared cargo value used to scale the tardiness penalty. Required at ingestion. (\emph{not a separate field in air MVP code} --- folded into \texttt{Hawb.tardiness\_weight} by the generator) & USD \\
Shipment-value coefficient & $\mu_k$ & $\mu_k = \text{value}_k / V^{\text{ref}}$ (dimensionless). Scales tardiness penalty in proportion to declared value. (\emph{not stored} --- folded into \texttt{Hawb.tardiness\_weight}) & --- \\
Combined tardiness weight & $W_k$ & $W_k = w_{\text{sp}(k)} \cdot \mu_k$. Per-HAWB coefficient in the quadratic tardiness term $W_k \cdot \tau_k^2$ (C.10, \S\ref{sec:lin-tardiness}). (code \texttt{Hawb.tardiness\_weight}) & USD / h\textsuperscript{2} \\
Origin door node & $O_k \in N_k$ & --- (code \texttt{door\_node(k,"O")}; \texttt{Hawb.origin\_gateway} / \texttt{origin\_door\_lat/lon} / \texttt{origin\_candidates}) & --- \\
Destination door node & $D_k^{\text{node}} \in N_k$ & --- (code \texttt{door\_node(k,"D")}; \texttt{Hawb.dest\_gateway} / \texttt{dest\_door\_lat/lon} / \texttt{dest\_candidates}) & --- \\
Port of discharge node & $\text{POD}_k \in N_k$ & HAWB-specific POD --- the air-arc head node where $k$'s air leg lands. Used in C.10a to define the terminal-arc set $\mathcal{A}^{\text{last}}_k$. Determined by the chosen route; multiple POD candidates may exist in $A_k$ pre-solve. (code: the chosen air arc's \texttt{head} node; graph-build) & --- \\
Post-POD ground tail transit sum & $\Delta^{\text{post}}_k$ & Sum of ground-arc dwells from $\text{POD}_k$ to $D_k^{\text{node}}$ on $k$'s destination tail: destination cartage, CFS dwell, customs clearance, final delivery. Used in C.10a to compute $\text{arr\_dest}(k, a) = \text{ETA}_a + \Delta^{\text{post}}_k$ for air terminal arcs. (code: precomputed into the terminal arc's \texttt{arr\_dest} at graph build; \texttt{\_dest\_tail\_transit}) & h \\
Per-HAWB customs dwell & $\delta^{\text{cust}}_k$ & Dwell time on the customs-clearance arc; depends on destination country, cargo type, commodity class, broker performance, pre-filed entry status (code \texttt{Hawb.customs\_dwell\_h}; cost \texttt{Hawb.customs\_cost}) & h \\
Per-HAWB AWB release dwell & $\delta^{\text{rel}}_k$ & Dwell at destination CFS between cargo arrival and AWB release. Depends on release type: \texttt{express\_release} $\approx 0$h; \texttt{telex} 1--4h; \texttt{original\_awb\_surrender} 24--72h. Folded into $\delta_a$ of the destination CFS dwell arc at graph build. (\emph{not a separate air MVP field} --- folded into the dest-CFS \texttt{GroundScalars.delta\_h}) & h \\
Cargo class & $\ctype_k$ & Canonical enum $\{\text{GEN, DGR, PER, VAL, AVI, HUM}\}$ following IATA Cargo-IMP (code \texttt{Hawb.cargo\_type}) & categorical \\
Temperature regime & $\text{temperature}(k)$ & $\{\text{ambient, chilled, frozen, pharma}\}$ (subdivides PER; other classes default to ambient) (code \texttt{Hawb.temperature}) & categorical \\
HS code & $\text{hs}_k$ & 6-digit Harmonized System code (\emph{not in air MVP code}) & --- \\
Incoterm & $\text{inco}_k$ & EXW / FCA / CIF / DDP / \ldots (\emph{not in air MVP code}) & --- \\
Service-product binding & $\text{sp}(k) \in P$ & The tenant service product the HAWB is contracted under (code: \texttt{Tier} on the \texttt{HawbArrival} stream; air-allow flag \texttt{Hawb.air\_allowed}) & --- \\
Lithium spec (optional) & $\text{lithium\_spec}_k$ & UN number, PI code, section, Wh, SOC compliance, DDR flag (\S\ref{sec:lithium}) (code: collapsed to the boolean \texttt{Hawb.has\_lithium}; per-leg \texttt{Leg.lithium\_ok}) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Parameter & Symbol & Description & Unit \\
\midrule
Dwell / transit time & $\delta_a$ & Per-arc dwell or transit (pickup truck, cartage, CFS build-up, deconsol-dwell, AWB release dwell, final delivery). Includes any in-transit hub customs adjustment for deconsol arcs. (code \texttt{arc.ground.delta\_h}, \texttt{GroundScalars}) & h \\
Flat per-HAWB handling charge & $c_a^{\text{flat}}$ & e.g.\ per-AWB CFS handling fee, deconsol fee. May be 0. (code \texttt{arc.ground.cost}; per-HAWB pickup/delivery from \texttt{Hawb.pickup\_cost} / \texttt{delivery\_cost}) & USD/HAWB \\
Per-kg handling charge & $c_a^{\text{kg}}$ & e.g.\ per-kg cartage. May be 0. (code: folded into \texttt{arc.ground.cost} at graph build --- no separate runtime field) & USD/kg \\
\bottomrule
\end{tabular}
\end{table}
...
Parameter & Symbol & Description & Unit \\
\midrule
Realized air transit time & $\mu_a$ & Graph-build-precomputed scalar. For multi-leg arcs (same-carrier connection or interline through-MAWB), $\mu_a$ already includes \emph{internal MCT} between legs --- no separate MILP constraint family for hub MCT. (code \texttt{arc.air.mu\_h}, \texttt{AirScalars}; scheduled \texttt{std\_utc\_h} / \texttt{eta\_utc\_h} = $\text{ETA}_a$) & h \\
Effective cargo cutoff at $\text{tail}(a)$ & $\text{CO}_a^*$ & Air arcs leaving a POL: $\max(\text{DCO}, \text{AMS}, \text{ICS2}, \text{ACI})$ across all legs of $a$, minus per-HAWB prep time. Graph-build-precomputed scalar. (code \texttt{Offer.cutoff\_utc\_h} / \texttt{arc.air.cutoff\_raw\_utc\_h} less \texttt{Hawb.prep\_time\_h}) & h (UTC) \\
Rate family & $\text{rate\_family}_a$ & $\{\texttt{flat\_rate}, \texttt{min\_flat\_breaks}, \texttt{per\_uld\_pivot}, \texttt{coload\_per\_kg}\}$. \texttt{coload\_per\_kg} applies only to $A^{\text{coload}}$. (code \texttt{Offer.rate\_family} / \texttt{arc.rate\_family}, enum \texttt{RateFamily}) & enum \\
Quotation currency & $\text{currency}_a$ & ISO 4217; converted to USD at MILP build. (\emph{not stored} --- conversion applied at build; rates enter in USD) & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Family & Symbols & Cost \\
\midrule
\texttt{flat\_rate} & $m_a$ (USD/kg), $\text{min\_chg}_a$ (USD per instantiated MAWB), $\text{cap}_a$ (kg, optional) & $\max(\text{min\_chg}_a \cdot z_{a,g},\, m_a \cdot \text{CW}_{a,g})$. Covers NAC, block rates, and dynamic spot quotes sold at one flat rate with no weight-break discount. \emph{Code:} \texttt{rates.flat[a]} = \texttt{FlatRate(m, min\_chg, cap)}. \\[0.3em]
\texttt{min\_flat\_breaks} & $\{(\text{break}_{a,b},\, \text{rate}_{a,b})\}_{b \in B_a}$, $\text{cap}_a$ (kg, optional) & IATA next-break-down: $\min_b \text{rate}_{a,b} \cdot \max(\text{CW}_{a,g},\, \text{break}_{a,b})$. Covers TACT GCR, SCR, CCR. $B_a$ ordered ascending; $\text{break}_{a,b}$ is the minimum billed weight if break $b$ is selected; $\text{rate}_{a,b}$ is the per-kg rate. \emph{Code:} \texttt{rates.min\_flat\_breaks[a]} = \texttt{list[Break(threshold, rate)]}. \\[0.3em]
\texttt{per\_uld\_pivot} (BSA) & $r_a$ (USD/kg) = $r_c$ for the tagging contract; $\pi_a$ (kg) pivot weight; $U_a \subseteq U$; $N_{a,u}$ (count) contracted ULD-position allotment; $W_u, V_u$ (kg, m\textsuperscript{3}) ULD physical capacity; $\text{settlement}_a \in \{\texttt{per\_flight}, \texttt{equalized}\}$ & $r_a \cdot \max(\text{CW}_{a,g},\, \pi_a \cdot \sum_u \eta_{a,g,u})$. BSA take-or-pay; pivot floor binds per flight for \texttt{per\_flight} settlement, or via the sunk allowance mechanism (C.13a) for \texttt{equalized}. \emph{Code:} \texttt{BsaContract(r\_a, pivot=}$\pi_a$\texttt{, allotment=}$N_{a,u}$\texttt{, settlement)} in \texttt{rates.bsa\_contracts}; $W_u, V_u$ = \texttt{UldType(w\_kg, v\_cbm)} in \texttt{rates.uld\_types}. \\[0.3em]
\texttt{coload\_per\_kg} & $m_a^{\text{cl}}$ (USD/kg), $\text{cap}_a^{\text{cl}}$ (kg, optional) & Per-kg, billed on each HAWB's own $cw_k$. \emph{Code:} \texttt{rates.coload\_per\_kg[a]} (USD/kg); spot CW cap (C.5d) \texttt{rates.spot\_cap[a]}. \\
\bottomrule
\end{tabular}
\end{table}
...
Parameter & Symbol & Description & Unit \\
\midrule
Per-kg contract rate & $r_c$ & Equal to $r_a$ for every $a \in A_c^{\text{MAWB}}$ (consistency assumption: all arcs tagged to one contract carry the same rate) (code \texttt{BsaContract.r\_c}; per-flight rate \texttt{r\_a}) & USD/kg \\
Remaining sunk allowance & $A_c$ & For $c \in C^{\text{eq}}$: $A_c = P_{c,t} - (\text{chargeable weight already consumed on $c$ this period})$, supplied exogenously per solve by the consumed-weight accumulator (\S\ref{sec:bsa-equalized-settlement}). Undefined for \texttt{per\_flight} settlement. (code \texttt{BsaContract.allowance\_kg}) & kg \\
\bottomrule
\end{tabular}
\end{table}
...
Parameter & Symbol & Description & Unit \\
\midrule
SLA transit-time bound & $T^{\text{SLA}}_p$ & For service product $p \in P$ (code: \texttt{TIER\_SPECS[tier]} SLA offset in \texttt{flexibility.py}; applied by \texttt{committed\_deadline}) & h \\
Tardiness weight & $w_p$ & Quadratic soft-tardiness penalty coefficient for service product $p$; combined with the per-HAWB value coefficient $\mu_k$ into $W_k = w_p \cdot \mu_k$ which scales $\tau_k^2$ in the objective (C.10, \S\ref{sec:lin-tardiness}). \texttt{CALIBRATION NEEDED} --- defaults pending. (code: \texttt{TIER\_SPECS[tier].w\_sp} in \texttt{flexibility.py}) & USD / h\textsuperscript{2} \\
\bottomrule
\end{tabular}
\end{table}
...
Parameter & Symbol & Description & Unit \\
\midrule
Dunnage factor & $\varepsilon$ & Dimensionless; default $\approx 0.05$ ($\approx 5\%$); used in $\text{Wt}_{a,g}$ aggregation (Eq.~\ref{eq:mawb-wt}) (code \texttt{MilpParams.dunnage\_eps}) & --- \\
MAWB fixed-charge & $c^{\text{MAWB}}_{\text{fix}}$ & Per-active-MAWB documentation overhead: customs manifest filing (AMS / ENS), MAWB issuance handling, and forwarder admin. \textbf{Scoped to ``new AWB number issued''} --- one charge per distinct MAWB-arc that triggers a fresh AWB filing, not per arc-activation when a single AWB spans multiple arcs (e.g.\ a future arc-splitting model edit must guard against double-charging the through-AWB case). Placeholder $\$50$/MAWB; \texttt{MARKET RESEARCH NEEDED}. Wired to $z_{a,g}$ in the objective (\S\ref{sec:objective}). \textbf{Invariant (S40): $c^{\text{MAWB}}_{\text{fix}} > 0$} --- a non-positive value is floored to \$1 (\texttt{MilpParams.\_\_post\_init\_\_}); this strict positivity is what makes C.2b redundant (\S\ref{sec:constraints}). Covers MAWB issuance + the \emph{master} manifest filing (AMS/ENS) only; \emph{per-HAWB} customs clearance is the separate $\delta^{\text{cust}}_k$/\texttt{customs\_cost} term on the customs arc, not this constant. (code \texttt{MilpParams.mawb\_fix\_cost}) & USD/MAWB \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Code map (flight metadata).} These live on \texttt{Leg} / \texttt{Offer} in
\texttt{air\_graph.py}, consumed at build, never by the MILP: $\text{ETD}_f/\text{ETA}_f$ =
\texttt{Leg.dep\_utc\_h} / \texttt{arr\_utc\_h}; $\text{ac\_type}(f)$ = \texttt{Leg.ac\_type}
(enum \texttt{AcType}); the DGR/PER/VAL/AVI/HUM capability flags = membership in
\texttt{Leg.cargo\_caps} (with \texttt{Leg.embargoed\_cargo} for embargoes);
$\text{lithium\_accept}_f$ = \texttt{Leg.lithium\_ok}; operating/marketing carrier =
\texttt{Leg.carrier} / \texttt{Offer.carrier}; the cutoff stack aggregates into
\texttt{Offer.cutoff\_utc\_h}. The forwarder-policy carrier sets ($C_k^{\text{allow/deny}}$) and
HS/incoterm are \emph{not} in the air MVP code.
...
Internal volume & $V_u$ & Usable interior after contour (code \texttt{UldType.v\_cbm}) & m\textsuperscript{3} \\
...
Cargo weight capacity & $W_u$ & $\text{MGW}_u - \text{tare}_u$; effective payload --- used in C.5b-w (code \texttt{UldType.w\_kg}; the air MVP \texttt{UldType} carries only \texttt{code}/\texttt{w\_kg}/\texttt{v\_cbm} --- MGW/tare/floor-load/contour are reference-only, not in code) & kg \\
...
\end{tabular}
\end{table}
\emph{Code:} channels are not a runtime enum --- each realizes as an \texttt{Offer} carrying a
\texttt{RateFamily} and its rate entry in \texttt{RateCatalog}; the spot CW cap (C.5d,
\texttt{rates.spot\_cap}) applies to every non-\texttt{per\_uld\_pivot} (spot) arc regardless of channel.
...
Parameter & Symbol & Description & Unit \\
\midrule
Tagged arcs & $A_c^{\text{MAWB}} \subseteq A^{\text{MAWB}}$ & MAWB-arcs whose offer is contract $c$ (code \texttt{BsaContract.arcs}) & --- \\
Per-arc allotment & $N_{a,u}$ & ULD-position count of type $u$ on arc $a \in A_c^{\text{MAWB}}$ (code \texttt{BsaContract.allotment[a][u]}) & ULDs \\
Pivot weight & $\pi_a$ & Minimum chargeable weight per ULD per flight under $c$ on arc $a$ (code \texttt{BsaContract.pivot}) & kg/ULD \\
Contract per-kg rate & $r_c$ & Equal to $r_a$ for every $a \in A_c^{\text{MAWB}}$ (code \texttt{BsaContract.r\_c}; per-flight rate \texttt{r\_a}) & USD/kg \\
Settlement basis & $\text{settlement}_a$ & $\{\texttt{per\_flight}, \texttt{equalized}\}$ --- per-arc attribute (a contract may have arcs of either basis, but typically uniform) (code \texttt{BsaContract.settlement}) & --- \\
Remaining sunk allowance (equalized only) & $A_c$ & Exogenous per solve from the consumed-weight accumulator (\S\ref{sec:bsa-equalized-settlement}); undefined for \texttt{per\_flight} (code \texttt{BsaContract.allowance\_kg}) & kg \\
Period commitment (equalized only) & $P_{c,t}$ & Period pivot weight $\times$ committed ULD count for period $t$; the take-or-pay base (\emph{not stored} --- exogenous; \texttt{allowance\_kg} already nets period-to-date) & kg \\
Per-arc secondary cap (optional) & $\text{cap}_a$ & BSA secondary actual-weight cap, enforced by C.5c (\texttt{C5c-uld}) (code \texttt{BsaContract.cap[a]}) & kg \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{Code (air MVP).} The MVP \texttt{Surcharge} dataclass collapses Path~A to the three live
bases: \texttt{rates.surcharges[a]} = \texttt{Surcharge(per\_kg, per\_shipment, per\_cbm)} (applied
on $cw_k$ / flat / $v_k$ in \texttt{\_set\_objective}). \texttt{per\_awb} $\equiv$
\texttt{per\_shipment} for single-HAWB AWBs; \texttt{per\_piece} and \texttt{percent\_of\_freight}
plus the scope/effective-window resolver (Eq.~\ref{eq:surcharge-applicable}) are \emph{not} in the
air MVP code. Path~B per-ULD is \texttt{rates.uld\_surcharge[a][u]}, billed on $\eta_{a,g,u}$.
...
Symbol & Description \\
\midrule
$K$ & Set of HAWBs (commodities, shipments) to route this solve, indexed by $k$ (code: the \texttt{hawbs} list arg; keys of \texttt{air\_graph.subgraphs}) \\
$N,\, A$ & All nodes and arcs of graph $G(N,A)$ (code: $A=$\texttt{air\_graph.arcs}, \texttt{dict[ArcId, Arc]}; $N=$ the arcs' \texttt{tail}/\texttt{head} endpoints) \\
$\text{tail}(a),\, \text{head}(a)$ & Endpoints of arc $a$ --- $a$ carries flow from $\text{tail}(a)$ to $\text{head}(a)$ (code \texttt{arc.tail} / \texttt{arc.head}) \\
$A^{\text{MAWB}} \subseteq A$ & MAWB-eligible air arcs --- $\text{rate\_family}_a \in \{\texttt{flat\_rate}, \texttt{min\_flat\_breaks}, \texttt{per\_uld\_pivot}\}$ (code: arcs appearing in \texttt{air\_graph.mawbs}) \\
$A^{\text{MFB}} \subseteq A^{\text{MAWB}}$ & Arcs with $\text{rate\_family}_a = \texttt{min\_flat\_breaks}$ (code \texttt{RateFamily.MIN\_FLAT\_BREAKS}; keys of \texttt{rates.min\_flat\_breaks}) \\
$A^{\text{coload}} \subseteq A$ & Co-load air arcs --- per-kg billing, no MAWB (code \texttt{RateFamily.COLOAD\_PER\_KG}; keys of \texttt{rates.coload\_per\_kg}) \\
$A^{\text{ground}} \subseteq A$ & Non-air arcs: pickup, cartage, CFS dwell, deconsol-dwell, customs-dwell, delivery (code: arcs with non-\texttt{AIR} \texttt{ArcType}) \\
...
$A_k \subseteq A$ & Pre-filtered subgraph of HAWB $k$ (\S\ref{sec:prefilter}) (code \texttt{air\_graph.subgraphs[k]}, \texttt{frozenset[ArcId]}) \\
...
$F$ & Set of physical flight legs (graph-build only --- the MILP does not index over $F$) (code \texttt{air\_graph.flight\_arcs} keys) \\
...
$U$ & Set of ULD types $\{\text{LD3, LD7, PMC, AKE,} \ldots\}$, indexed by $u$ (code \texttt{rates.uld\_types}, \texttt{dict[str, UldType]}) \\
$U_a \subseteq U$ & ULD types the forwarder can claim on \texttt{per\_uld\_pivot} arc $a$. Determined by the forwarder's BSA contract allotment ($N_{a,u} > 0$) (code: keys of \texttt{contract.allotment[a]}), bounded above by the ULD positions the underlying aircraft physically supports. Single-forwarder solve: no cross-tenant pooling \\
$C$ & Set of \texttt{per\_uld\_pivot} (BSA) contracts, indexed by $c$ (code \texttt{rates.bsa\_contracts}, \texttt{list[BsaContract]}; $c \leftrightarrow$ \texttt{contract.id}) \\
$C^{\text{eq}} \subseteq C$ & Equalized-settlement BSA contracts (code \texttt{contract.settlement == "equalized"}) \\
$A_c^{\text{MAWB}} \subseteq A^{\text{MAWB}}$ & Arcs tagged to BSA contract $c$ (code \texttt{contract.arcs}) \\
$A^{\text{pu}} = \bigcup_{c \in C} A_c^{\text{MAWB}} \subseteq A^{\text{MAWB}}$ & Per-ULD-pivot arcs (code: keys of \texttt{arc\_contract}, built by \texttt{\_bsa\_context}) \\
$g(k)$ & Group key of HAWB $k$ as defined by the function $g(\cdot)$ (\S\ref{sec:g-of-k}) (code: the \texttt{GroupKey} $g$ in the \texttt{(a,g)} keys of \texttt{air\_graph.mawbs}); the range of $g$ is only ever iterated via $G_a$ (below), never globally \\
$K_a = \{k : a \in A_k\}$ & HAWBs whose pre-filtered subgraph contains arc $a$ (code \texttt{air\_graph.riders[a]}, \texttt{frozenset[HawbId]}). Solve-specific (depends on this solve's HAWB batch) \\
$G_a = \{g(k) : k \in K_a\}$ & Distinct group keys present on arc $a$, derived by projecting $K_a$ through $g(\cdot)$ (code: the $g$'s among \texttt{air\_graph.mawbs} keys sharing arc $a$). Not chosen by the optimizer; computed at build time. Solve-specific \\
$M = \{(a, g) : a \in A^{\text{MAWB}},\, g \in G_a\}$ & MAWB candidates --- the central index set (code \texttt{air\_graph.mawbs}, \texttt{dict[(ArcId,GroupKey), Mawb]}; drives the $z$/$\text{CW}$ columns). One $(a, g)$ pair per (MAWB-eligible arc) $\times$ (group present on that arc), enumerated at build. Solve-specific \\
$B_a$ & Weight-break segments of a \texttt{min\_flat\_breaks} arc $a$, ordered ascending; indexed by $b$ (code \texttt{rates.min\_flat\_breaks[a]}, \texttt{list[Break]}; $b \leftrightarrow$ list index $i$) \\
$P$ & Tenant catalog of service products, indexed by $p$ (\S\ref{sec:service-products}) (code: \texttt{Tier} enum + \texttt{TIER\_SPECS} in \texttt{flexibility.py}) \\
$\text{sp}(k) \in P$ & Service product bound to HAWB $k$ (code: the HAWB's \texttt{Tier}, on the \texttt{HawbArrival} stream) \\
$C_k^{\text{allow}},\, C_k^{\text{deny}}$ & Resolved hard-allowed / hard-denied carrier sets for HAWB $k$ from the policy cascade (\S\ref{sec:carrier-policy}). \emph{Not in the air MVP code} --- carrier policy is applied at graph-gen pre-filter, not in \texttt{air\_milp} (deferred, N11). \\
$\text{chargeable}(c)$ & Derived continuous (\(\geq 0\)) for $c \in C^{\text{eq}}$: sum of MAWB-level chargeable weights tendered on contract $c$ this solve, $\sum_{(a,g):\, a \in A_c^{\text{MAWB}},\, g \in G_a} \text{CW}_{a,g}$ (code: \texttt{chargeable} local in \texttt{\_build\_c13a\_equalized} / \texttt{\_validate\_bsa}). Used in C.13a to compute overage. \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
$x_{k,a}$ & binary & HAWB $k$ uses arc $a$ (code \texttt{x[(k,a)]}, \texttt{h.addBinary}) & $k \in K$, $a \in A_k$ \\
$z_{a,g}$ & binary & MAWB $(a, g)$ is instantiated (active) (code \texttt{z[(a,g)]}) & $(a, g) \in M$ \\
$\text{CW}_{a,g}$ & continuous $\geq 0$ & Chargeable weight booked on MAWB $(a, g)$ (code \texttt{cw[(a,g)]}) & $(a, g) \in M$ \\
$\text{Wt}_{a,g}$ & \emph{(inlined)} & Aggregate actual weight on MAWB $(a, g)$ (with dunnage). \textbf{Not a variable in code:} inlined as the linear expression \texttt{wt} inside \texttt{\_build\_c4\_density} (C.4a's equality only ever feeds C.4c). & $(a, g) \in M$ \\
$\text{Wv}_{a,g}$ & \emph{(inlined)} & Aggregate volumetric weight on MAWB $(a, g)$. \textbf{Not a variable in code:} inlined as \texttt{wv} inside \texttt{\_build\_c4\_density} (C.4b's equality only ever feeds C.4d). & $(a, g) \in M$ \\
$\gamma_{a,g,b}$ & binary & Break selector for MAWB $(a, g)$ at break $b$ (\texttt{min\_flat\_breaks} arcs only) (code \texttt{gamma[(a,g,i)]}, index $b\!=\!i$) & $(a, g) \in M,\, a \in A^{\text{MFB}},\, b \in B_a$ \\
$\text{BW}_{a,g,b}$ & continuous $\geq 0$ & Disaggregated billed weight at break $b$ (code \texttt{bw[(a,g,i)]}) & as $\gamma$ \\
$\eta_{a,g,u}$ & integer $\geq 0$ & ULDs of type $u$ claimed for MAWB $(a, g)$ (code \texttt{eta[(a,g,u)]}) & $(a, g) \in M,\, a \in A^{\text{pu}},\, u \in U_a$ \\
$\text{pivot}_{a,g}$ & continuous $\geq 0$ & Pivot-active billed weight $\max(\text{CW}_{a,g},\, \pi_a \cdot \sum_u \eta_{a,g,u})$ (code \texttt{pivot[(a,g)]}) & $(a, g) \in M,\, a \in A^{\text{pu}},\, \text{settlement}_a = \texttt{per\_flight}$ \\
$\text{over}_c$ & continuous $\geq 0$ & Allowance overage for equalized contract $c$: $\max(0,\, \sum_{(a,g)} \text{CW}_{a,g} - A_c)$ (code \texttt{over[c]}) & $c \in C^{\text{eq}}$ \\
$t_k^{D_k^{\text{node}}}$ & continuous $\geq 0$ & Arrival time of HAWB $k$ at its destination node; defined by C.10a from precomputed $\text{arr\_dest}(k, a)$ scalars (code \texttt{t\_var[k]}, named \texttt{t[k]}). Intermediate-node arrival times do not appear in the MILP --- see \S\ref{sec:fwd-time-propagation}. & $k \in K$ \\
$\tau_k$ & continuous $\geq 0$ & Tardiness of HAWB $k$ against $\Delta_k$ (code \texttt{tau[k]}) & $k \in K$ \\
$\text{pen}_k$ & continuous $\geq 0$ & PWL outer-approximation of the quadratic tardiness penalty $W_k \cdot \tau_k^2$; appears in the objective in place of $\tau_k^2$ (\S\ref{sec:lin-tardiness}) (code \texttt{pen[k]}) & $k \in K$ \\
\bottomrule
\end{tabular}
\end{table}
...
Symbol & Description \\
\midrule
$K^{\text{fb}}$ & HAWBs routed via the fallback arc: $\{k : x_{k, a^{\text{fb}}_k} = 1\}$ (code \texttt{AirSolution.fallback\_hawbs}; routes \texttt{.routes}, active MAWBs \texttt{.active\_mawbs}, per-MAWB CW \texttt{.mawb\_chargeable\_weight}, ULD counts \texttt{.mawb\_uld\_counts}, overage \texttt{.bsa\_overage}, tardiness \texttt{.hawb\_tardiness\_h}) \\
$z^{\text{routing}}$ & Real routing cost: sum of all objective terms \emph{excluding} fallback-arc cost and tardiness penalty. The number operators see on planning screens as ``cost to serve.'' (\emph{not a separate field} --- derivable from the routes; the MVP \texttt{AirSolution} surfaces only the total) \\
$z^{\text{tardiness}}$ & Total quadratic tardiness penalty $\sum_k \text{pen}_k$ (PWL approximation of $\sum_k W_k \tau_k^2$) (\emph{not a separate field}) \\
$z^{\text{fallback}}$ & Fallback usage cost $C^{\text{fallback}} \cdot |K^{\text{fb}}|$ (\emph{not a separate field}) \\
$z^{\text{total}}$ & Solver objective $= z^{\text{routing}} + z^{\text{tardiness}} + z^{\text{fallback}}$ (code \texttt{AirSolution.total\_cost}; solver diagnostics \texttt{.status} / \texttt{.mip\_gap} / \texttt{.dual\_bound} / \texttt{.num\_col} / \texttt{.num\_row}) \\
\bottomrule
\end{tabular}
\end{table}
...
deterministic $t_k^{D_k^{\text{node}}}$ in MVP. \textbf{Service reliability is
controlled upstream at graph generation} (predicate~9, \S\ref{sec:prefilter}:
the tier-reliability admission filter, consuming a deterministic per-route
reliability scalar from \texttt{air\_transit\_time.md}), \emph{not} by binding a
quantile into the MILP; the in-MILP P85--P90 quantile swap
(\ref{item:tt-quantile-binding}) is \textbf{not in MVP}.
...
by pruning arcs that fail any of nine predicates. This is Phase~1 step~11 of
...
$T_k^{\text{abs}}$ & Latest valid arrival time (fallback-arc arrival; predicate 7 reachability bound) & \S\ref{sec:hawb-params} \\
$\Delta_k$ & Effective soft deadline $\min(T_k^{\text{dead}}, T_k^{\text{SLA}})$ (predicate 9 reliability bound) & \S\ref{sec:hawb-params} \\
...
$\hat{A}(r),\, \hat{\sigma}(r)$ & Deterministic estimate end-to-end arrival / dispersion proxy of candidate route $r$ (predicate 9) & \texttt{air\_transit\_time.md \S6} \\
$z_{\text{tier}(k)}$ & Per-service-tier reliability safety multiplier (predicate 9) & \texttt{air\_transit\_time.md \S5} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Forward-time-window admission.} For each candidate arc $a =
        ...
        (iii) otherwise propagate the window forward.
  \item \textbf{Destination reachability.} Retain $a$ only if a forward path
        from $n_2 = \text{head}(a)$ to $D_k^{\text{node}}$ exists with cumulative
        arrival $\leq T_k^{\text{abs}}$ (otherwise $a$ is dominated by the
        fallback on both cost and arrival). The fallback arc is exempt from this
        and every other pre-filter predicate.
  ...
  \item \textbf{Service-tier reliability admission (OTP control).} For tiered
        service products, admit a \emph{candidate route} $r$ (an enumerated
        end-to-end $O_k \to D_k^{\text{node}}$ path in the air candidate set ---
        the air per-shipment path set is small) only if its \emph{deterministic}
        reliability margin clears the tier floor:
        \begin{equation}
          \hat{A}(r) \;+\; z_{\text{tier}(k)} \cdot \hat{\sigma}(r) \;\leq\; \Delta_k,
          \label{eq:tier-reliability-ok}
        \end{equation}
        where $\hat{A}(r)$ is the estimate end-to-end arrival, $\hat{\sigma}(r)
        \approx \sqrt{\sum \sigma^2}$ a deterministic end-to-end dispersion proxy
        (both from the transit-time model, \texttt{air\_transit\_time.md \S6}),
        $z_{\text{tier}}$ the per-tier safety multiplier, and $\Delta_k =
        \min(T_k^{\text{dead}}, T_k^{\text{SLA}})$ the effective soft deadline
        (\S\ref{sec:hawb-params}; the same bound C.10 penalizes against). This
        predicate runs \emph{last} (Phase~1 step~11) over the already-pruned $A_k$,
        as a path-level screen over enumerated candidate routes projected back onto
        arcs --- routes are already feasible on predicates 1--8 before reliability
        screening. An arc is retained in $A_k$ only if it lies on at least one
        tier-admissible route. \textbf{This is the primary OTP control}: express
        tiers admit only reliable/fast routes, deferred tiers the full set. It is computed \emph{deterministically} --- no Monte-Carlo, and
        \emph{no per-arc quantile propagation} (the convolution problem,
        \S\ref{sec:fwd-time-propagation}; cf.\ \ref{item:tt-quantile-binding}); a
        single end-to-end scalar per candidate route is evaluated once at build.
        It is an \emph{approximate screen}: $\hat\sigma(r) \approx \sqrt{\sum
        \sigma^2}$ is conservative for slack-rich routes but can \emph{under-state}
        dispersion for tight-connection routes (the cascade is super-additive ---
        a small upstream slip blows a connection $\to$ a discrete downstream jump),
        so it can false-admit a fragile route. Realized OTP is an \emph{ex-post
        population metric} (\texttt{backtest\_methodology.md \S6}), and
        $z_{\text{tier}}$ is tuned \emph{closed-loop} against it (covering exactly
        this failure direction) and \emph{frozen per shipment at booking}. Because
        this is an \emph{arc} filter, the MILP may still recombine surviving arcs
        into a path that was not itself tier-admissible; the screen is a
        \emph{candidate-set prune}, with the selected path's reliability backstopped
        ex-post by the C.10 penalty, the closed-loop $z_{\text{tier}}$, and the
        fallback (exact per-path admission is the deferred path-based migration,
        \ref{item:tt-quantile-binding}). Per-tier filtering can make members of a
        shared consolidation group $g$ (\S\ref{sec:g-of-k}, keyed on
        class/temperature, \emph{not} tier) diverge across arcs --- an express HAWB
        is excluded from a low-reliability MAWB leg its filter pruned while a
        deferred group-mate stays; this is intended (the group is not forced to move
        as a unit). This is \textbf{not} a chance constraint (those are deferred,
        not in MVP --- \ref{item:probabilistic-transit}). \emph{Build note: not yet
        implemented; documented here as the chosen OTP-control mechanism.}
\end{enumerate}
...
\begin{equation*}
  \text{reject\_record}(k, a) \;\defeq\; \bigl(k,\; a,\;
    \text{predicate}_{1\ldots 9},\; \text{evidence}\bigr)
\end{equation*}
where $\text{predicate}_{1\ldots 9}$ is the failing pre-filter predicate
index (1 = lane / reachability, 2 = cargo type, 3 = embargo, 4 =
lithium, 5 = service product, 6 = forward-time-window admission, 7 =
destination reachability, 8 = per-HAWB ULD fit, 9 = service-tier
reliability admission) and $\text{evidence}$
...
Active constraint families: C.1, C.2, C.4, C.5/C.5b/C.5c/C.5d, C.10, C.12 (pins),
C.13, C.14. Time propagation, cargo cutoffs, and hub MCT are enforced at graph build via
forward-time-window propagation (\S\ref{sec:fwd-time-propagation}), not in the MILP.

\paragraph{Code map (constraint families $\to$ builders).}
Every family below is built by a dedicated \texttt{\_build\_c*} function in
\texttt{src/components/air\_milp.py}, and each constraint row carries the bracketed
\emph{code label} shown in its header as its HiGHS row name (visible in the
\texttt{air\_model.lp} dump produced under \texttt{MilpParams(debug=True,\ output\_lp\_file=True)}).
Decision-variable code names are in Table~\ref{tab:decision-variables}. The orchestration order
is fixed in \texttt{solve()}; constraint-row and column order are canonicalized with
\texttt{sorted()} (+ \texttt{PYTHONHASHSEED=0}) for cross-process determinism.
...
\subsection*{C.1 Flow conservation \quad{\normalfont(code \texttt{C1[k,n]}, \texttt{\_build\_c1\_flow})}}
...
\subsection*{C.2 MAWB instantiation linkage \quad{\normalfont(code \texttt{C2a[a,g,k]}, \texttt{\_build\_c2\_mawb\_linkage})}}
...
\end{equation}
C.2a forces MAWB activation when any HAWB rides the arc in that group. It is the
\emph{disaggregated} (per-HAWB) linkage --- the strong fixed-charge form, not the weak
$\sum_k x_{k,a} \le |K_a|\, z_{a,g}$.

\paragraph{C.2b dropped (S40).} The converse ``no phantom activation'' bound
\begin{equation}
  z_{a,g} \;\leq\; \sum_{k \in K_a : g(k) = g} x_{k,a}
  \tag{C.2b, removed} \label{eq:C2b}
\end{equation}
is \textbf{not built}. It is redundant given the \texttt{MilpParams} invariant $c^{\text{MAWB}}_{\text{fix}} > 0$
(\S\ref{sec:air-arc-params} ``Other''): every active MAWB then carries strictly positive cost, so the
objective drives $z_{a,g}=0$ on an empty MAWB --- C.2a alone yields $z_{a,g} = 1 \iff \exists k,\, x_{k,a}=1$
at the optimum. C.2b was also \emph{non-binding} at the LP optimum ($z_{a,g} = \max_k x_{k,a} \le \sum_k x_{k,a}$),
so it never tightened the relaxation. The only corner it guarded --- a \emph{cost-free} active MAWB
(equalized BSA with zero overage \emph{and} $c_{\text{fix}}=0$) --- is closed by the invariant, which
floors a non-positive $c_{\text{fix}}$ to \$1 rather than rejecting it.

\subsection*{C.4 Chargeable-weight aggregation (density mixing) \quad{\normalfont(code \texttt{C4c[a,g]} / \texttt{C4d[a,g]} / \texttt{C4ub[a,g]}, \texttt{\_build\_c4\_density})}}
...
\textbf{Code note.} $\text{Wt}_{a,g}$ and $\text{Wv}_{a,g}$ are \emph{not} materialized as
variables; \texttt{\_build\_c4\_density} inlines C.4a/C.4b as the linear right-hand-side
expressions \texttt{wt}/\texttt{wv} of the two emitted rows \texttt{C4c}/\texttt{C4d}. The
upper-link $\text{CW}_{a,g} \leq \text{CW}^{\text{ub}}_{a,g}\cdot z_{a,g}$ (C.14 domain,
Eq.~\ref{eq:cw-ub}) is emitted here as row \texttt{C4ub[a,g]}.

\subsection*{C.5 Per-contract allotment cap (\texttt{per\_uld\_pivot}) \quad{\normalfont(code \texttt{C5[a,u]} / \texttt{C5act[a,g,u]}, \texttt{\_build\_c5\_allotment})}}
...
\subsection*{C.5b Per-ULD physical capacity (item 13-A fix) \quad{\normalfont(code \texttt{C5bw[a,g]} / \texttt{C5bv[a,g]}, \texttt{\_build\_c5b\_uld})}}
...
\subsection*{C.5c Per-offer cap (when specified) \quad{\normalfont(code \texttt{C5c[a]} / \texttt{C5c-uld[a]}, \texttt{\_build\_c5c\_caps})}}
...
cap skip C.5c. The code emits \texttt{C5c[a]} for \texttt{flat\_rate} arcs with a cap and
\texttt{C5c-uld[a]} for a BSA secondary per-arc cap; both bound \emph{actual} weight $w_k$.
...
\subsection*{C.5d Spot per-arc chargeable-weight cap \quad{\normalfont(code \texttt{C5d-spot[a]}, \texttt{\_build\_spot\_cap})}}

\textbf{Added S39 (F1 Slice C, D-A19).} Each spot arc $a$ (any \emph{non-}\texttt{per\_uld\_pivot}
channel: \texttt{co\_load}, \texttt{flat\_rate}, \texttt{min\_flat\_breaks}) carries a
chargeable-weight ceiling $\text{cap}^{\text{spot}}_a$. \emph{Every} spot channel must be capped:
an uncapped channel is an infinite escape hatch that would zero the supply scarcity the
replan-savings proof measures (independent-supply model, methodology \S13). The cap reads
chargeable weight from whichever variable the arc's billing uses:
\begin{align}
  \sum_{k \in K_a} cw_k \cdot x_{k,a} &\;\leq\; \text{cap}^{\text{spot}}_a
    && \text{co-load (per-kg) arc, on } x \tag{C.5d-cl} \label{eq:C5d-cl} \\
  \sum_{g \in G_a} \text{CW}_{a,g} &\;\leq\; \text{cap}^{\text{spot}}_a
    && \text{MAWB (\texttt{flat\_rate} / \texttt{min\_flat\_breaks}) arc, on } \text{CW} \tag{C.5d-mawb} \label{eq:C5d-mawb}
\end{align}
Distinct from C.5c, which caps \emph{actual} weight $w_k$; C.5d caps \emph{chargeable} weight
$cw_k$ / $\text{CW}_{a,g}$. Contracted (\texttt{per\_uld\_pivot}/BSA) arcs are never spot and
carry no \texttt{spot\_cap} entry. The generator draws $\text{cap}^{\text{spot}}_a$ and a
two-sided rate multiplier on the $\kappa$-independent \texttt{spot\_regime} RNG stream so the
$\kappa$ sweep moves contracted tightness only.

\subsection*{C.10 Destination arrival, soft deadline, quadratic tardiness \quad{\normalfont(code \texttt{C10a[k]} / \texttt{C10b[k]} / \texttt{PWL[k,j]}, \texttt{\_build\_c10\_tardiness})}}
...
\paragraph{$W_k$ as a per-shipment control input.}
$W_k$ is not only a static service-product weight: it is a \emph{per-shipment
control input}. Its default is a fixed, pre-registered per-tier ratio (via
$w_{\text{sp}(k)}$), but it is \emph{exposed as an external signal} to prioritize
specific shipments under capacity contention --- e.g.\ raising $W_k$ for a customer
who is behind on month-to-date OTP so a scarce reliable slot breaks toward them (a
displacement-cost / bid-price signal; same external-control-input pattern as the
equalized-BSA accumulator, \ref{item:volume-kicker-accumulator}). It shapes
\emph{who wins scarce capacity}, not only lateness magnitude. \textbf{OTP
differentiation is owned by the graph-gen tier-reliability filter (predicate~9,
\S\ref{sec:prefilter}), not by $W_k$}; $W_k$ shapes prioritization and lateness
within the admitted set. Note the metric/objective gap: the quadratic minimizes
\emph{mean tardiness}, which is correlated with but not identical to the
\emph{binary} on-time count (\texttt{backtest\_methodology.md \S6} reports both to
guard against reducing mean tardiness while worsening the on-time fraction). For
the replan-savings proof this control input is \emph{frozen} (no live
OTP-to-date$\to$$W_k$ loop), so the headline number is not confounded by tuning the
reported metric.
...
$\text{arr\_dest}(k, a)$ scalars. \textbf{Service reliability is controlled
upstream at graph generation} (predicate~9, \S\ref{sec:prefilter}: the
tier-reliability admission filter), \emph{not} by binding a quantile into the
MILP. The in-MILP end-to-end-quantile swap (\ref{item:tt-quantile-binding}) is
\textbf{not in MVP} --- reserved for contractual per-shipment SLAs and thin-tail
lanes (see that item).

\subsection*{C.12 Pins --- forced commitments \quad{\normalfont(code \texttt{pin[k,a]}, \texttt{\_build\_pins})}}

\textbf{Reframed S39 (D2/D-A11).} Earlier drafts treated locks as pure preprocessing
(``not constraints''). The replay machinery instead imposes them as explicit equality rows.
For each pinned pair $(k, a)$ in the caller-supplied set $\text{Pin} \subseteq K \times A$:
\begin{equation}
  x_{k,a} \;=\; 1 \qquad \forall (k, a) \in \text{Pin}
  \tag{C.12} \label{eq:C12-pin}
\end{equation}
Pins are the mechanism the arrival-replay loop uses to (a) \textbf{lock tendered bookings} (no
arm may reshuffle a firm shipment once its cutoff has fired) and (b) \textbf{freeze prior-cycle
commitments} for the no-reshuffle arms $M_0$/$M_1'$ (\texttt{arrival\_only\_replan\_methodology.md},
D-A11). \texttt{solve(pinned=\ldots)} adds one \texttt{pin[k,a]} row per pair whose $x_{k,a}$
variable exists; a pin whose arc has dropped from the HAWB's subgraph (its planned route went
infeasible) has no variable and is \emph{silently skipped} --- the caller routes that HAWB to its
fallback arc.

\paragraph{Equivalent to variable-fixing.} Imposing $x_{k,a}=1$ as a constraint and fixing the
variable to $1$ by substitution describe the \emph{same} feasible region (e.g.\ a cap
$\sum_{k'} cw_{k'} x_{k',a} \le \text{cap}$ with a pinned 300\,kg HAWB is the explicit row plus
$x_{k,a}=1$, identical to the substituted $\sum_{k'\ne k} cw_{k'} x_{k',a} \le \text{cap}-300$).
Writing it as a row keeps the model body uniform and self-documenting; HiGHS presolve fixes and
removes the column anyway, so there is no solve-time cost. The locked-state pre-MILP reachability
check remains an early warning, not a feasibility gate.

\subsection*{C.13 BSA settlement}

\subsubsection*{C.13a Equalized-settlement allowance \quad{\normalfont(code \texttt{C13a[c]}, \texttt{\_build\_c13a\_equalized})}}
...
\subsubsection*{C.13b Per-flight settlement pivot \quad{\normalfont(code \texttt{C13b1[a,g]} / \texttt{C13b2[a,g]}, \texttt{\_build\_c13b\_pivot})}}
...
\begin{align*}
  ...
  \text{CW}_{a,g} &\in [0,\, \text{CW}^{\text{ub}}_{a,g} \cdot z_{a,g}] & & \forall (a, g) \in M \\
               & & & \text{(empty bucket} \Rightarrow \text{CW} = 0;\ \text{code row } \texttt{C4ub[a,g]}) \\
  \text{Wt}_{a,g},\,\text{Wv}_{a,g} & \text{ --- inlined, no column} & & \text{(RHS expressions of \texttt{C4c}/\texttt{C4d}; see C.4 code note)} \\
...
\begin{align}
  ...
  & \sum_{(a,g) \in M} c^{\text{MAWB}}_{\text{fix}} \cdot z_{a,g}
    \tag{MAWB fixed-charge (issuance + master manifest filing; per-HAWB customs is separate)} \\
  +\;
...
\paragraph{Code map (objective).} The whole objective is assembled term-by-term into the
\texttt{terms} list and set via \texttt{h.minimize(sum(terms))} in \texttt{\_set\_objective}: bucket
cost = \texttt{c\_flat} (\texttt{\_build\_flat\_bucket\_cost}) $+$ \texttt{mfb\_terms}
(\texttt{\_build\_min\_flat\_breaks\_cost}) $+$ \texttt{r\_a·pivot} (\texttt{\_build\_c13b\_pivot}
vars); MAWB fixed-charge = \texttt{params.mawb\_fix\_cost · z}; co-load = \texttt{rates.coload\_per\_kg};
ground = \texttt{arc.ground.cost}; BSA overage = \texttt{r\_c·over}; Path-A surcharge =
\texttt{rates.surcharges}; Path-B = \texttt{rates.uld\_surcharge · eta}; tardiness = $\sum$\texttt{pen}
(\texttt{\_build\_c10\_tardiness}); fallback rides the fallback arc's \texttt{ground.cost}. The
post-solve monotonicity / billing invariant is checked by \texttt{\_validate\_billing} and
\texttt{\_validate\_bsa}, which recompute each active MAWB's closed-form tariff cost from the
realized routing and compare it to the solver's billing variables (\texttt{\_check\_billing}).

\textbf{Optimality-gap-aware tolerance (S41).} The check is asymmetric.
\emph{Under}-billing --- a realized cost \emph{below} the tariff floor --- means a broken
linearization (e.g.\ a negative-coefficient term inflating $\text{CW}$, breaking monotonicity)
and is raised unconditionally. \emph{Over}-billing --- a MAWB sitting on a non-cost-minimal
break or bucket --- is a defect \emph{only at a proven-exact optimum}, where the solver would
have polished it away; the check therefore fires only when the returned gap is exact
($\text{mip\_gap} \leq 10^{-9}$). This corrects a latent bug: since the solver stops at the
deterministic relative-gap tolerance $\text{mip\_rel\_gap} = 0.005$ (\S\ref{sec:scaling-roadmap}),
the \texttt{kOptimal} status no longer implies exactness --- a within-gap incumbent can
legitimately leave a MAWB one break above its cheapest (cost difference $\leq$ the gap, hence
immaterial to $Z^*$). The MFBlink cut (Eq.~\ref{eq:mfb-link}) made this surface in practice: by
closing the bound so fast that HiGHS reaches the $0.5\%$ gap before polishing a non-binding
break choice. Keying the over-billing branch off the status string \texttt{OPTIMAL} (pre-S41,
which also tolerated the \texttt{TIME\_LIMIT} incumbent path) crashed \texttt{solve()} on exactly
that case; keying it off the actual $\text{mip\_gap}$ is the correct test and subsumes the
\texttt{TIME\_LIMIT} tolerance.
...
\paragraph{Solver settings (code, \texttt{MilpParams}).}
\texttt{solve()} pins \texttt{threads=1} + \texttt{random\_seed=0} for cross-process determinism
(with \texttt{PYTHONHASHSEED=0}). The tenant scalars in this section map to \texttt{MilpParams}:
$\varepsilon$ = \texttt{dunnage\_eps}, $c^{\text{MAWB}}_{\text{fix}}$ = \texttt{mawb\_fix\_cost}, the
PWL tangent grid = \texttt{tardiness\_alphas}. Two solve-control knobs govern termination (S39):
\texttt{mip\_rel\_gap} (default \texttt{0.005}) is a \emph{deterministic} 0.5\% relative-gap stop ---
the sound criterion for any cost that enters the replan-savings (L2) decomposition, since a
gap-based stop is reproducible across machines; \texttt{time\_limit\_s} (default \texttt{600}) is a
\emph{soft} wall-clock backstop (HiGHS checks it only at node boundaries, so real wall time can
overshoot --- accepted). A \texttt{kTimeLimit} return with an incumbent surfaces as
\texttt{AirSolution.status="TIME\_LIMIT"} with the residual \texttt{mip\_gap} / \texttt{dual\_bound}
reported (vs.\ \texttt{"OPTIMAL"} when the gap tolerance is met, \texttt{"INFEASIBLE"} otherwise).
Two debug switches (default off, zero overhead): \texttt{debug} names every variable / constraint in
the HiGHS model with its tex-matching label, and \texttt{output\_lp\_file} additionally writes
\texttt{air\_model.lp} (LP-name-sanitized: \texttt{[ ] : -} $\to$ \texttt{( ) . \_}) so the dump
reads 1:1 against this document.
...
\subsection{Bucket cost linearization \quad{\normalfont(code \texttt{\_build\_flat\_bucket\_cost} / \texttt{\_build\_min\_flat\_breaks\_cost})}}
\label{sec:lin-bucket}

\paragraph{\texttt{flat\_rate}. {\normalfont(code: aux \texttt{cflat[a,g]}, rows \texttt{FLATmin[a,g]} / \texttt{FLATcw[a,g]})}}
...
\paragraph{\texttt{min\_flat\_breaks} (TACT / SCR / CCR --- item 7). {\normalfont(code: vars \texttt{gamma[a,g,i]} ($\gamma$) / \texttt{bw[a,g,i]} (BW); rows \texttt{MFBsel} (one-break) / \texttt{MFBcap} (BW$=$0) / \texttt{MFBfloor} (BW$\geq$break) / \texttt{MFBcw} (BW$\geq$CW) / \texttt{MFBlink} ($\Sigma$BW$\geq$CW, Eq.~\ref{eq:mfb-link}); \texttt{\_build\_min\_flat\_breaks\_cost})}}
...
\paragraph{LP-strengthening cut: $\sum_b \text{BW}_b \geq \text{CW}$ (code \texttt{MFBlink}, S41).}
The three-inequality form above is exact at every integer point, but its
\emph{LP relaxation} is severely loose. With the break selector
$\gamma_{a,g,b}$ fractional, the \texttt{MFBcw} rows $\text{BW}_{a,g,b} \geq
\text{CW}_{a,g} - M^{\text{BW}}_{a,g}(1 - \gamma_{a,g,b})$ go slack --- the
big-M term $M^{\text{BW}}(1-\gamma_b)$ dominates --- so each $\text{BW}_b$ can
fall to its break-floor fraction $\text{break}_b \gamma_b$ and the billed cost
$\sum_b \text{rate}_b \text{BW}_b$ collapses far below any integer cost. This is
the dominant source of root integrality gap on proof-scale instances. We add
the valid aggregate link inequality
\begin{equation}
  \sum_{b \in B_a} \text{BW}_{a,g,b} \;\geq\; \text{CW}_{a,g}
  \qquad \forall (a, g) \in M,\ a \in A^{\text{MFB}}.
  \label{eq:mfb-link}
\end{equation}

\textbf{Validity (excludes no integer-feasible point).} At any integer point
exactly one $\gamma_{a,g,b^*} = 1$; its \texttt{MFBcw} row collapses to
$\text{BW}_{b^*} \geq \text{CW}_{a,g}$, while every other $\text{BW}_b = 0$
(forced by \texttt{MFBcap}), so $\sum_b \text{BW}_b = \text{BW}_{b^*} \geq
\text{CW}_{a,g}$. When $z_{a,g} = 0$ then $\text{CW}_{a,g} = 0$ and the row is
$0 \geq 0$. Eq.~\ref{eq:mfb-link} is therefore \emph{redundant at integer
feasibility} --- a pure cut on the relaxation, leaving the integer optimum
(and, with the fixed seed and \texttt{sorted()} row order, cross-process
determinism) unchanged. The per-break \texttt{MFBcw} rows are kept: the cut
bounds the \emph{sum}; they pin the \emph{selected} bucket to its round-up floor.

\textbf{Why it bites --- numeric example.} Take the worked-check breaks
$(b=45,\ \text{rate}=\$10/\text{kg})$ and $(b=100,\ \text{rate}=\$8/\text{kg})$
with $\text{CW} = 90$ on an active MAWB ($z=1$, so $\gamma_{45}+\gamma_{100}=1$).
\emph{Any} integer plan pays \$800 (round up to $b=100$: $\text{BW}_{100}=100$,
$8 \cdot 100$). But the relaxation may split $\gamma_{45} = \gamma_{100} =
\tfrac{1}{2}$: then \texttt{MFBfloor} gives only $\text{BW}_{45} \geq 45 \cdot
\tfrac{1}{2} = 22.5$ and $\text{BW}_{100} \geq 100 \cdot \tfrac{1}{2} = 50$,
while both \texttt{MFBcw} rows are slack, so the LP bills $10(22.5) + 8(50) =
\$625$ --- 22\% below every integer plan. Eq.~\ref{eq:mfb-link} forbids this
point: it requires $\text{BW}_{45} + \text{BW}_{100} \geq 90$, but the cheat
sums to $72.5$. The cheapest repair loads the $17.5$ deficit onto the
lower-rate break ($\text{BW}_{100} = 67.5$), lifting the relaxation to
$10(22.5) + 8(67.5) = \$765$. The residual \$35 is the convexification of the
discrete break choice. On the proof-scale cells the cut closes $\approx 98\%$
of the root integrality gap (seed-2 $(\kappa,\alpha)=(1,1)$: $31.5\% \to 0.55\%$;
monolithic solve $\sim\!110\text{s} \to 0.1\text{s}$), $\kappa$-invariant and
replicated across seeds, with $Z^*$ unchanged.

$M^{\text{BW}}_{a,g}$ is the per-MAWB-tight big-M --- see
\S\ref{sec:bigm-tightening}. (Empirically, shrinking $M^{\text{BW}}$ gives
\emph{zero} bound lift on these instances: the looseness is the missing
aggregate link Eq.~\ref{eq:mfb-link}, not the big-M magnitude.)
...
\subsection{Pivot weight linearization (per-flight BSA) \quad{\normalfont(code: var \texttt{pivot[a,g]}; rows \texttt{C13b1[a,g]} / \texttt{C13b2[a,g]}, \texttt{\_build\_c13b\_pivot})}}
\label{sec:lin-pivot}
...
\subsection{Quadratic tardiness linearization (outer approximation) \quad{\normalfont(code: vars \texttt{t\_var[k]} / \texttt{tau[k]} / \texttt{pen[k]}; rows \texttt{C10a[k]} / \texttt{C10b[k]} / \texttt{PWL[k,j]}, \texttt{\_build\_c10\_tardiness}; cuts indexed by \texttt{MilpParams.tardiness\_alphas})}}
...
$\min_b \text{rate}_b \cdot \max(\text{CW},\, \text{break}_b)$ & \texttt{min\_flat\_breaks} & Binary $\gamma_b$ exactly-one-if-active ($\sum \gamma = z$) $+$ \textbf{3-inequality} $\text{BW}_b$ disaggregation (no $\text{BW} \leq \text{CW}$) $+$ aggregate link cut $\sum_b \text{BW}_b \geq \text{CW}$ (Eq.~\ref{eq:mfb-link}, LP-tightening, $\approx\!98\%$ of root gap). Verified worked example. \\
...
\begin{itemize}[noitemsep]
  ...
  \item predicate 9 (service-tier reliability) failure: no route clears
        the tier's reliability floor; downgrade the service tier (relax
        $z_{\text{tier}}$ / accept lower OTP) or procure more reliable
        capacity.
\end{itemize}
...
\begin{enumerate}
  \item \label{item:tt-quantile-binding} \textbf{TT-Service P85--P90 quantile
        ...
        \textbf{NOT IN MVP (Session 29).} The chosen MVP OTP-control mechanism is
        the deterministic \emph{graph-gen tier-reliability admission filter}
        (predicate~9, \S\ref{sec:prefilter}), which consumes a per-route
        end-to-end reliability scalar from the transit-time model
        (\texttt{air\_transit\_time.md \S5}) at build --- no in-MILP quantile
        binding. This item is deferred (not built), reserved for the regimes the
        portfolio filter does not cover: contractual per-shipment SLAs, thin-tail
        lanes, and singleton cargo (VAL/HUM/AVI).
  ...
  \item \label{item:probabilistic-transit} \textbf{Probabilistic transit
        ...
        tardiness. \textbf{NOT IN MVP (Session 29).} Superseded for the common
        case by the graph-gen tier-reliability admission filter (predicate~9,
        \S\ref{sec:prefilter}) plus the per-shipment $W_k$ prioritization control
        input --- OTP is an ex-post population metric, not a per-shipment chance
        constraint (\texttt{backtest\_methodology.md \S6}). Reserved for
        contractual per-shipment SLAs, thin-tail lanes, and singleton cargo where
        portfolio averaging fails.
...
\midrule
$|A_k|$ histogram + per-predicate drop counts (the 9 pre-filter predicates) & \texttt{pre\_filter\_stats.jsonl} per HAWB & Determines $x_{k,a}$ binary count. The single most important number. \\
...
\begin{enumerate}
  \item \label{item:scale-prefilter} \textbf{Pre-filter aggressiveness has
        100$\times$ leverage; instrument first.} The pre-filter applies nine
