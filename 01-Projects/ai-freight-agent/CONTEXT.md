...
**Last updated:** 2026-07-11 (Session 54 — **2D SPOT RESERVATION: grounded (6 agents), designed (replaces D-A1), 7-agent verified (2 rounds), slice S1 BUILT. 386 green.** Grounded the 3 spot rate-families → flat/MFB collapse to one channel; decay is a channel property (code already correct); reserve-then-assign real for spot+coload general cargo (identity-lock = min(cutoff, ACAS pre-load)). Designed the 2D weight+volume spot reservation replacing locked D-A1: reserve-early/assign-late, **sunk-cost/free-reserved-capacity** cost (user caught my broken scored-friction model), 2D cap `C^v=C^w/333.33` (honest ~2× tighter volume → recalibrate via τ), `penalty_frac=1` (NC-a dismissed as domain non-issue), S51 M1′-dump reversal blessed. Verified across 7 code-reading agents; folded into spec REV 3. Built slice S1 (schema split `spot_cap`→`spot_wcap`/`spot_vcap`, byte-identical). **NEXT: slice S2 (reservation state + ratchet) → S3–S7.** Spec `docs/design/spot_reservation_2d_design_s54.md` (REV 3); resume anchor memory `project_s54_2d_spot_reservation`. — Prior S53: 2026-07-10 (Session 53 — **TIMESTAMP REMODEL Phase 0+1 BUILT, 386 green.** Grounded the ⟨book, available, latest-arrival⟩ triplet (8 research agents); built the grid-realism unit: tier = SLA slack (deferred = standard+3d) + the existing tardiness weight (no new "uplift" axis — base_transit is already door-to-door + tier-agnostic); opt-in `flight_horizon_days` (16d flights / 7d demand so deferred can reach later flights); random `book_at` metadata via an independent CRN-safe `book_gap` RNG stream (nothing consumes it yet — data capture for the future anticipatory pre-buy model). Reverted the broken book-curve-on-`known_at` edit → D-A9 restored. 4 plan-checkers + 2 reviewers vetted the plan (caught a generator crash + a methodology pivot). **NEXT: Phase 2 — M1′ coherence (symmetric reserve floor), the S52 structural fix.** Plan of record `scratchpad/timestamp_remodel_plan.md` v3; grounding `docs/design/air_cargo_demand_arrival_grounding.md`. — Prior S52: **DESIGN + GROUNDING, NO BUILD. Synthesized the commit-timing fix: split `tender` into reserve-space-early + assign-cargo-late (symmetric arms). 5-agent deep-dive (industry consultant / OR scientist / air planner / airline RM / red-team) converged on the reserve-vs-assign primitive; red-team reframe = make M1′ coherent FIRST (it currently commits a route early but holds no space → forced fallback dump; symmetric reserve for both arms). Proposal written to `model/commit_timing_reserve_assign_proposal.tex` (NOT compiled). Two rounds of grounding research: (1) BSA-heavy deep-research (workflow harness HUNG twice on a dead URL `scm-en.ecer.com` — WebFetch no-timeout; recovered by hand from 64 cached verified claims); (2) SPOT-specific two-agent cross-check (Q1/Q2/Q3). VERDICT: reserve-space-early/assign-cargo-late IS standard for spot (master booking cargo-agnostic — route/commodity/weight + own AWB #, shipper/consignee only for special cargo; IATA Cargo-IMP FFR≠FWB split, IAG e-booking, cargo.one). Friction REFINED: cutoff-gated fractional no-show penalty (AA $300>250kg; LH 25%<48h/50%<24h), free to release before cutoff — NOT flat full-spot charge → reserve likely releasable, not monotone. 48h cliff = calibration range not constant. Full synthesis (grounding table G1–G9 + changes table C1–C8 + 5 open decisions) saved to memory `project_s52_reserve_assign_synthesis` (CURRENT resume anchor). New: `docs/design/air_cargo_reserve_assign_grounding_s52.md`, memory `reference_air_cargo_reserve_assign_practice`. NOTHING built; awaiting user sign-off on the 5 decisions, then one consistent doc pass, then build (M1′ coherence first). Agent-created reference files (`papers/`, `docs/references-air-cargo-two-stage-allotment.md`, edits to 2 reference docs) left UNCOMMITTED/unreviewed — triage next session.** ) Prior S51:) 2026-07-05 (Session 51 — **BUILT & COMMITTED (386 passed, ruff clean, 2 independent reviews): increment 4 freighter repositioning (D-A30), cleanup, fix-B per-origin BSA backstop, F1 grounded convex decay (ONLY SPOT DECAYS; both BSA tiers firm; `A_cut` right-skewed Beta freighter 0.13/belly 0.22), Model-Y (items 2+3 — spot floor tendered-only for M1/M1'/π_hind + M1' rigid dump-to-fallback + fail-fast guard), soft-BSA release cliff (item 1 — firm until dep−48h then lock-used/release-unused), §14.2 rewrite + 2 new design docs. THEN the D3-sweep DIAGNOSTICS pivoted the whole thing: feasibility table (every HAWB has in-time routes → timing wall ruled out) + perfect-info clairvoyant solve (9% fb/60% OTP) + no-decay staged M1 ≈ clairvoyant (7% fb/63% OTP) PROVED that 100% of the staged degradation is the DECAY and the real bug is ARRIVAL/COMMIT TIMING, not the decay: cargo arrives ~2d before cutoff (median book-lead 1.9d) and commits AT the cutoff, so it never catches spot (fills over ~2 weeks; φ≈15% at commit vs ≈56% at ~5d out). Research (grounded, agents): real book-lead ~5d not 2d; distress-pricing = NO reliable close-in markdown (don't model it → book early); L2_real% is a mirage under Model-Y (retire it, use service+total). ▶ Resume the OPEN design discussion (commit-timing + BSA-swap allocation), NOT building. See SESSION_LOG S51 + memories `project_s51_decay_timing_modely` / `reference_air_cargo_timing_and_pricing`.**) Prior S50:) 2026-07-04 (Session 50 — **D3 sweep re-run for corrected (provisional) L1 → L2_real median +16.5% (n=70, reproduces S49); L1_real −27.5% is the D5 mirage (H0 strands 60–81%). Real finding: fallback 30–70% in EVERY cell, 100% capacity_roll (0% structural) — the SHORT region was starved even at τ=1.05. Geography audit (4 subagents) → TWO defects: (a) origin truck-drayage crossed the Taiwan Strait (100% of HAWBs, no land/water test) — FIXED standalone, COMMITTED `7c08539` (`freightnet.ground_group` + geo_select seed gate; 0 cross-water arcs, L2 STRENGTHENED +16→+33%); (b) region incoherence (LAX+SFO one market split short/slack; no East Coast). §14.1-R region redesign APPROVED v1.0 D-A25..D-A30: candidacy=door-distance not clustering (D-A25/26); regions=supply-indexing (D-A25b); dest=3 metros West/Midwest/East (D-A27); origin=3 regions Taiwan/PRD/EastChina (D-A28); tightness re-keyed to O-D LANE origin-dominant WIDE/NARROW bands q_ij=q^O·q^D Σ=τ (D-A29); belly+BSA lane-frozen, freighter sole repositionable pool `reposition_rho` default 0, supply⊥demand preserved (D-A30; design doc `docs/design/s50_lane_tightness_freighter_repositioning.md`, sourced). BUILT slice 1 taxonomy + slice 2+3 O-D-lane re-key + schedule (gateways DTW/JFK/EWR + grounded East 112h transit; `lane_of`, `_draw_lane_*`, `_build_lane_rate_catalog`, `_draw_lane_doors`; demand spreads across all 9 lanes). 374 passed, ruff clean, CRN verified (demand ⟂ τ/α), lane path solves OPTIMAL (~80% fallback = pre-calibration operating point, deferred). ▶ Resume increment 4 (freighter repositioning D-A30) → cleanup dead region funcs → calibrate operating point → re-run D3 sweep = thesis number.**) Prior S49:) 2026-06-30 (Session 49 — **air build Phases 1c+D2+2+3 BUILT & calibrated; D3 sweep → L2_real ≈ +9–18% at proof scale (n≈70). Cost-split D5 (real_cost vs fallback_penalty, never summed) + tardiness ALWAYS-ON calibration (`CALIBRATED_TARDINESS_W`, generator default ON — structural guardrail, memory `feedback_tardiness_penalty_always_on`) + `CapDecay`. H0 human baseline REBUILT (greedy supply-first batch consolidation + knob-C reroute repair, `src/h0_planner.py`) — feasible + capacity-responsive, chain C(H0)≥C(M1') holds on total cost, BUT strands ~29–51% → redesign PARKED (`EXECUTION_PLAN.md`), awaits user pseudocode; L2 (machine-vs-machine) unaffected. Tractability deep-dive (4 OR agents), 2 problems: (A) `x` was O(H²) from a HKG dwell-arc leak → FIXED to O(H) (subgraph keeps own dwell arc, optimum bit-identical) SHIPPED; (B) ~7952s blowup = weak-LP dual-bound on integer `eta`, driven by n (~90–100 phase transition) + mid-tight tau (NOT hard_bsa), FIX1 (eta MIR cut) is R2 research, interim = looser `mip_rel_gap` at large n (26× faster / 0.14% obj) KEEPING 0.005 for L2 cells. Committed `c8bb738` (infra) + `ee365fc` (H0+FIX2); suite 358 green; reusable probes kept in `scripts/`. ▶ Resume: re-run D3 sweep for the corrected L1 (human-vs-machine) number now H0 is fixed; H0 redesign on user pseudocode; FIX1 + symmetry-breaking = R2.**) Prior S48:) 2026-06-28 (Session 48 — **planning only: air-vertical product roadmap R1–R5 +
autonomous-loop policy for LCL recorded (`EXECUTION_PLAN.md` § "2026-06-28 — Air-vertical product
roadmap"); R1 remaining-work pace estimate computed from git-dated cadence (**2.5–3 weeks** to the air
thesis number); memory `project_autonomous_loop_lcl` written. No code touched; air build position
UNCHANGED, resume at Phase 1 slice 1c. See SESSION_LOG S48.** Prior S47:) 2026-06-27 (Session 47 — **Phase 0 of the S46 build COMPLETE: §14 methodology amendment drafted in `arrival_only_replan_methodology.md`, D1–D5 walked one-by-one with the user and APPROVED v1.0; §13 reconciled (D-A18/D-A19/D-A7 superseded → pointers); banner flipped. Locked: D1 flat-spot+decay (block→v2) · D2 BUILD belly/freighter+deferred-air now (Phase 1 grows) · D3 SWEEP both settings (composition {0.55,0.70} / hard_bsa {0.35,0.50} / τ {1.0–1.15,≤0.8}) · D4 both OTP forms · D5 TWO cost numbers real_cost+fallback_penalty kept SEPARATE, never summed (ψ_cap/capacity-consolidation split RETIRED, weight-break-confounded). No code touched. ▶ Phase 1 (generator) STARTED.** Prior:) 2026-06-27 (Session 46 — **S46 capacity redesign: 7-agent design fan-out (`docs/design/s46_capacity_redesign/` 01–06) → red-team flagged 2 FATAL S45-reproduction risks → user's TEMPORAL insight (time-decaying capacity + within-tier arrival spread) became the binding mechanism → kill-shot (`scripts/killshot_s46.py` + minimal default-off `cap_decay` hook on `run_replay`) BUILT & PASSED across 5 seeds, W on and off: the two replan arms route DIFFERENT chargeable-kg per capacity tier (the scalar S45 never computed; there it was $0) ⇒ capacity binds; open-book M1 dominates cost/fallback/tardiness-severity. W-on (calibrated tier-quadratic tardiness, was inert) lifts OTP into the realistic 80–92% band. DECAY MODEL CORRECTED with the user (a modeling error — decaying soft-BSA): only the FREE pool decays, booked capacity fixed, used ULDs counted PER commodity-group; verified feasible 70/70 all 5 seeds decay-on. FULL BUILD SCOPED: Phase 0 methodology gate → 1 generator (τ_ℓ+mix+hard-BSA+arrival-spread) → 2 decay in real planner → 3 scorer/metrics → 4 calibrate+sweep → 5 tractability; decisions D1–D5; recommended LEAN path. Suite 311 green, replay.py ruff-clean. Kill-shot is SCRATCH. ▶ S47 STARTS at Phase 0 = draft methodology amendment with D1–D5 for one-pass approval.** Prior:) 2026-06-26 (Session 45 — **Standing review agents (overdue since S43) → A1 (κ-inert L2) + A2 (gap noise floor) VERIFIED with own eyes → L2 decomposition proved the air replan value is CONSOLIDATION reshuffle, NOT capacity → a finite increasing-block spot-pricing redesign was designed, pressure-tested, and SHELVED as not-the-fix. No `src/`/`tests/` code changed; suite unchanged at 311 passed, ruff clean.** Critiques `docs/critique/18` (calibration) / `19` (seam) / `20` (red-team). **A1:** L2 byte-identical across κ∈{0.5,1,2,8} for 5/6 seeds (mean ~$377→$386; κ=8 ≠ ≈0) — the κ sweep axis is INERT. **A2:** L2 is a difference of two 0.5%-gap objectives, the ~$180 gap budget overlaps L2 ($137–$1056); seed-5 negative L2 vanishes at mip_rel_gap=1e-4, seed-3 M₀<M₁′ persists (realized-vs-planned nesting, not gap). **SEAM BLOCKING:** `route_eta_under_disruptions` returns `broken=True` for routes `route_reliability` admits via `max(dep,clock)` catch-next-cycle ⇒ the §6 disruption gate silently NO-OPS for 1–7/12 routes/scenario (a cancel/+500h delay triggers no replan) — fix before any more 2c-7. **L2 decomposition (`docs/design/l2_decomposition_s45.md`):** across seed1/2/0, M₁ and M₁′ route byte-identical total spot kg, ZERO contracted, ZERO fallback ⇒ **L2 is 100% consolidation reshuffle (fewer/fatter MAWBs into cheaper IATA weight-breaks + gateway swaps), 0% capacity.** Contracted is NEVER used: BSA pivot floor ~1250 kg/ULD makes it ~$5.7/kg for ~700 kg lane cargo (worse than spot) + supply⟂demand mismatch scatters positions onto demand-less flights. **A1 is a SCALE problem, not pricing.** The increasing-block curve prices total-kg-per-lane (held equal by both arms) ⇒ amplifies L2 by $0. Pricing work (`docs/design/air_pricing_calibration_s45.md` sourced transpac ~$5.5 spot/$4.2 contract + `air_pricing_capacity_redesign_s45.md` convex/no-binary/BLK-1c-safe + `air_spot_composition_research_s45.md` + `redesign_pressuretest_s45.md` which also found OPEN-2's undiscounted-base premium INVERTS the weight discount) is sound but SHELVED. Confirms `project_air_replan_value_source` value-half (re-consolidate the open book), refutes its capacity-sweep half. Memory `project_l2_decomposition_s45` written; review-cadence bumped S45→S52. **▶ S46: ASK the Stage-3 fork FIRST (deferred — user tired): A re-axis to arrival-density/α/λ [recommended] / B forwarder-scale / C both. Then the SEAM gate bug + A2 tight-gap fix.** **Prior:** Session 44 — **H₀ daily cadence + 2c-7 disruption recourse slices 1 & 2a + recourse tardiness calibration. 311 passed, ruff clean.** (1) **H₀ now plans once a day** (`_daily_times`: 24h grid + short-fuse ad-hoc booking); cadence is **arm-split** — machine arms (M0/M1p/M1/PIH) stay event-driven, H₀ daily; tender rule generalized to `tender_at < next_t` (proven byte-identical for the event-driven arms). `_plan_cycle_h0` now places every newly-arrived shipment per daily run. (2) **Disruption injection + §6 gate:** `Disruption(flight_id, kind∈{delay,cancel}, delay_h=delta, realized_at)`; `run_replay(..., disruptions=None, M1-only)`; new `route_eta_under_disruptions` (HARD-departure walk: a delayed flight that lands too late MISSES the connection; reduces to `route_reliability`'s mean with no disruption ⇒ headline inert); `_refresh_active_shipments` gates on the **disruption's EFFECT** (replan iff it newly breaks a feasible route OR degrades a was-on-time route — NOT an already-late shipment). (3) **Recovery (slice 2a):** `_disrupted_sim` bakes the disruption into the world (threaded loop-wide); the flagged shipment is unlocked from its flown prefix (`_flown_prefix`, now **departed-by-decision-clock** per §6.1: anchor = head of the last DEPARTED arc), recourse-W stamped, re-routed by the M1 solve, re-tendered; arcs resolved from a stable undisrupted `ag_orig`; `handled` set stops re-flagging. **Demonstrated:** cancel a flight ⇒ reroute via alt dest gateway to the SAME door, prefix preserved, **promise Δ_k frozen**, deterministic. (4) **Recourse tardiness penalty CALIBRATED (OR subagent):** tier-quadratic, reuses the EXISTING C.10 PWL — W_EXPRESS=1.3/W_STANDARD=0.65/W_DEFERRED=0.32 ($/h², ratio 4:2:1), anchored so EXPRESS full-span ≈ fallback gap; flat per-tier (not ·weight_kg). **Finding: the $40k fallback (critique-13 N1) is STALE — live `FallbackPolicy` ≈ $16-18k.** **Methodology §6.1 ADDED** (mid-shipment recourse anchoring + node-anchored fallback, for BOTH sim AND production planner). **▶ NEXT: 1b node-anchored fallback** (graph injection + gate fix for non-flagged shipments whose pinned flight is pruned in sim_t + a multi-arc test bed — seed-3 has none) → 2b ledger conservation → re-measure MFBlink → Stage 3. **Two reconcile flags: D-A14 "batch-cutoff H₀"→daily; the $40k fallback drift.** Ledger still audit-only. Prior:) 2026-06-24 (Session 43 — **2c-6 BUILT: H₀ human baseline + π_hind clairvoyant LB — the last two replay arms. 304 passed, ruff clean. `_ARMS = {M0,M1p,M1,H0,PIH}`.** Designed H₀ via a subagent (`docs/design/h0_human_consolidation_policy.md`) after the user rejected two AskUserQuestion framings — the key correction: **a human ALWAYS consolidates** (fills ULDs, chases break weights); H₀'s suboptimality is **myopia** (no joint cross-shipment optimization, no reshuffle, no lookahead), NOT failure to consolidate. **H₀ (`_plan_cycle_h0`):** batch-at-cutoff (D-A14); each shipment routed by its own cheapest STANDALONE route (a singleton `solve([k])` — one box ⇒ zero multi-shipment tradeoff), priors frozen; the committed book billed by ONE all-pinned solve (consolidation/break-rating/ULD-packing emerge from co-routed cargo — no second billing code path to drift). Capacity contention (naive standalone routes over-subscribe a capped arc ⇒ INFEASIBLE all-pinned solve) handled by **fallback-roll** (roll marginal batch shipments largest-cw-first to the fallback arc until feasible; MILP is the feasibility arbiter, no ledger). **π_hind (`PIH`):** one full-book cycle (cadence collapsed to terminal time), no pins ⇒ clairvoyant optimum; dispatches through the M1 single-solve branch. **TWO FINDINGS banked for Stage 3 (user choice 'c'):** (1) **L1 can be ≤ 0 — timing, not a bug:** H₀ commits at cutoff (more info), M₁′ at reveal (less) ⇒ H₀'s late-commit can beat M₁′ (measured L1≈−$258 at seed-2 κ=2, mirror of L2≈+$258); D-A15 anticipates this (H₀ timing set L2-favorable, NOT a claimed bound) but §4 line~59 still writes the chain as guaranteed — reconcile by reporting L1 honestly (value lives in C(M₀)−C(M₁′)) and/or adding on-arrival H₀. (2) **fallback-roll is harsh** (fallback=T^abs late+~$40k); roll-to-next-cheapest-FLIGHT is the deferred realism refinement; π_hind is a LB only within mip_rel_gap(0.5%). **▶ SIGNED OFF mid-2c-7 design — see RESUME HERE for the OPEN QUESTION to ask FIRST on resume.** Prior:) 2026-06-23 (Session 42 — **2c replay machinery BUILT: 2c-3 `run_replay` loop + 2c-4 M₀/M₁′/M₁ arms + 2c-5 `score_run` scorer, all in `src/replay.py` (+ `tests/test_replay_loop.py`, 16 tests). 296 passed, ruff clean.** The three replan arms + the scorer now exist (BLK-1c being resolved made the per-cycle solves sub-second, which unblocked this). **2c-3** = event-driven cadence (sorted `known_at` ∪ `tender_at`), build-over-visible-subset, hard-pin on tender, §7 snapshots (`planning_runs`/`routes`/`route_legs`), audit-only ledger (`tendered=0`, whole claim → `committed_untendered`; the tendered/untendered ULD-split is deferred to 2c-7 where it feeds back). **2c-4** = `arm ∈ {M0,M1p,M1}` dispatch — M1 open-book (pin tendered only), M1p single-pass (pin all prior placements ⇒ freeze-from-first-placement), M0 greedy (newcomers one-at-a-time, unprocessed excluded, then a final all-pinned eval solve); cost chain `C(M0)≥C(M1p)≥C(M1)` verified with strict divergence at seed-2 κ=2 tier-coupled (~$258). **2c-5** = realized arrival via `route_reliability` (the deterministic Â; s=0, no RNG reopened), OTP = `A≤Δ_k` vs the FROZEN `booking_promise`, realized cost = a final all-pinned solve (excludes C.10, inert at W=0), fallback⇒arrives `T^abs`; writes `realized`+`metrics`; the cross-arm L1/L2 + L2_reshuffle/L2_fallback split correctly stays Stage 3 (`metrics.l1`/`l2_*` left NULL). `src` stays free of a module-level `data.synthetic` dep (build/load imported locally in the loop). **▶ NEXT: 2c-6** H₀ heuristic + π_hind → **2c-7** recourse fixtures (where the ULD-ledger split lands) → re-measure MFBlink lift across (κ,α,λ) cells (BLK-1c caveat) → **Stage 3** sweep = the thesis number. **Standing review agents due S43.** Prior:) 2026-06-23 (Session 41 — **BLK-1c RESOLVED on the hard cells: MFBlink LP cut + gap-aware billing-validator fix + C.10 skip. 280 passed, ruff clean. Committed `27ea807`.** Scalability review of the air formulation: measured (not guessed) the seed-2 (κ=1,α=1) root integrality gap at **31.5%** via `scripts/lp_gap_probe.py` — and the per-family fractionality showed **η fully integral at the root** (C.5b/C.13b NOT the culprit; my prior guess was wrong), the looseness all in `z`/`γ`/`x`. An OR deep-dive subagent isolated the **entire** gap to the `min_flat_breaks` big-M bucket-weight disaggregation. Fix = the **MFBlink cut** `Σ_b BW_{a,g,b} ≥ CW_{a,g}` (one valid row per MFB MAWB; redundant at integer feasibility ⇒ pure cut, Z* unchanged): **31.5%→0.55%, 106s→0.1s**, replicated seed-0 (97.5%), κ-invariant; all 6 hard seed-2 cells now solve in **0.2s** at the 0.5% gap. Also fixed a latent billing-validator bug the cut surfaced (`_check_billing` keyed off `status=="OPTIMAL"`, but `mip_rel_gap=0.005` makes `kOptimal` mean within-gap → a gap-stopped incumbent can over-bill a MAWB and crash `solve()`; now gates over-billing on the actual `mip_gap`, exact ⟺ ≤1e-9, threaded through the validators). And skipped the C.10 tardiness build entirely when all W_k=0 (proof default; −45 cols/−105 rows seed-2). tex reconciled (MFBlink + 90kg/$625→$765 numeric example §lin-bucket; gap-aware validator note §objective). **PDF now 3 compiles behind (S38+S39+S41) — NOT recompiled.** Commit `27ea807` ALSO picked up previously-uncommitted **S40 code** (`MilpParams.__post_init__` c_fix>0 floor + C.2b-drop + 2 tests) — the S40 sign-off staged only trackers + 2c-2 data/scripts/tests, never `air_milp.py`/tex/test (staging gap). 6 scratch scripts left untracked per user. **Instance-specificity caveat: MFBlink lift confirmed on seed-2 + seed-0 only — re-measure across more (κ,α,λ) cells before fully locking BLK-1c.** **▶ NEXT: 2c-3 replay-loop skeleton (M₁ open-book), now UNBLOCKED.** Prior:) 2026-06-17 (Session 40 — **2c-2 `tender_at` wiring DONE (committed `45528c0`); 278 passed, ruff clean.** Binding cutoff (D-A1) wired generator→persistence: `HawbArrival.tender_at` = `d_star.cutoff_utc_h` (stable across the pass-2 `latest_ready` clamp); `scenario_io._persist_hawbs` writes the existing `shipments.tender_at` column; round-trip + CRN-signature + lifecycle-invariant (`known_at ≤ ready_at ≤ tender_at ≤ Δ_k < backstop`) tests; ruff-cleaned `scripts/`. `load()` still rebuilds only `Hawb` — replay reads `tender_at` off the column (deferred arrivals-inverse stands). **▶ NEXT: 2c-3 replay-loop skeleton (M₁ open-book; solve to `mip_rel_gap=0.005`, not a time cap).** Prior:) 2026-06-17 (Session 39 — **BLK-1c tractability probe disproved the S38 safety-valve + `mip_rel_gap=0.005` deterministic stop + model naming/.lp dump + status-aware billing-validator bug fix + value-bound test tolerance + full LaTeX code-annotation reconcile. 278 passed, ruff clean. Committed 79c5ac7 + sign-off trackers.** (1) **Probe (`scripts/`):** time-limited incumbents are seed-/machine-load-dependent (non-deterministic), the wall-clock cap is soft (120s→200s), and the residual gap (5–9%) is same-order-as-L2 → the S38 safety-valve is UNUSABLE for the proof; seed 2 is the consistently hard instance (20s→157s; κ/α barely move it — it's consolidation branching, BLK-1b); instance size 73 nodes/240 arcs/176 MAWBs/2075 cols×2803 rows. (2) **Solver policy:** `MilpParams.mip_rel_gap=0.005` = a DETERMINISTIC 0.5% relative-gap stop, the sound termination for any cost entering L2; `time_limit_s` demoted to soft backstop; `mip_gap` now the real residual; +`dual_bound`/`num_col`/`num_row`. (3) **Bug fix:** `_validate_billing`/`_validate_bsa` asserted exact billing (valid only at optimality) → over-billing on a TIME_LIMIT incumbent crashed `solve()` intermittently; fixed with status-aware `_check_billing` (under-billing always raises, over-billing tolerated when not OPTIMAL). (4) **Tests:** `tests/conftest.within_rel()` 0.5% band on 26 objective value-bound assertions (D1=global 0.005, no test broke; determinism/relational/difference/physical kept exact). (5) **Model naming (D4):** `debug`/`output_lp_file` name every var+constraint with tex labels → 1:1 `air_model.lp` (LP-sanitized `[ ] : -`→`( ) . _`; default off, zero overhead). (6) **LaTeX (D1/D2):** added **C.5d spot cap** (`_build_spot_cap`), **reframed C.12→pins** (`_build_pins`, D-A11), documented Wt/Wv inlining + solver settings; `(code …)` annotations across sets/variables/post-solve + ALL parameter tables + every C.x constraint + objective + linearization; PDF intentionally not recompiled (now 2 behind). **▶ RESUME: 2c-2 `tender_at` → 2c-3 replay loop (solve to the 0.5% gap, NOT a time cap) → arms → scorer → Stage 3; BLK-1c gates Stage 3.** Prior:) 2026-06-15 (Session 38 — **critique-17 CLEARED + M₀/M₁′ decomposition reshape + F1 Slice C + N3 ReplayState + 2c-1 pinning. 278 passed, ruff clean.** (1) **Reading B locked:** M₀=greedy / M₁′=single-pass-optimal / M₁=open-book; chain `C(H₀)≥C(M₀)≥C(M₁′)≥C(M₁)≥C(π_hind)`; **L1=C(H₀)−C(M₁′)** (user choice; M₀ internal ablation), **headline L2=C(M₁′)−C(M₁)** (intra-engine, artifact-free); retired the `C(M₁′)==C(M₀)` leakage placebo. All methodology/backtest/thesis/flex/human-heuristic/scenario-io docs reconciled. (2) **BLK-1 mitigated:** 600s HiGHS time-limit + incumbent (`MilpParams.time_limit_s`, `AirSolution.mip_gap`, `status="TIME_LIMIT"`); real tractability deferred; chain holds per-draw only for OPTIMAL solves. (3) **BLK-2 dismissed** (non-bug): predicate-8 guarantees routable HAWBs fit 1 ULD → `⌈cw/1000⌉` already dominates; critique conflated group-η with alone-η. (4) **cheap fixes:** MAT-4 t_dead floor 96h / MIN-3 Δ_k<T^abs assert / MIN-4 radius guards / MAT-5 empty-seed raises / MIN-2 v1-drift scrub / MAT-2 "second-order" retract / MAT-3 born-at-risk diag / MIN-1 φ=1.3 load-bearing confirmed (PVG→HKG→LAX 1.240). (5) **F1 Slice C:** `spot_cap` + C.5d caps CW on BOTH coload & flat/MFB (uncapped=escape hatch); generator draws cap `U(1,3)×1500` + two-sided rate `base×U(.85,1.18)` on κ-independent `spot_regime` stream; persists via rate_json. (6) **N3 `ReplayState`** (`src/replay.py`): in-memory monotonic clock + parameterized reveal + **declarative-reconcile ledger** (two units: contracted INT `capacity_ledger`, spot REAL `spot_capacity_ledger` ε-conserv) + `tendered_set()` pin hook; design vetted by an independent agent (3 BLOCKING in v1 → fixed). (7) **2c-1:** `solve(pinned=…)` hard-pins x=1 (D-A11). **Memory to add S39:** Reading-B decomposition + L1=H₀−M₁′. Prior:) 2026-06-14 (Session 37 — **graph-gen workstream BUILT + a 4-agent critique that WALKED BACK the B5 "resolved" claim.** 255 passed, ruff clean. Built, in order: **`FreightNet`** (`src/freightnet.py` — physical freight-node reference DB, all 9 node types, 3,274 real airports from an OurAirports CSV asset; spatial service nodes_within_km/nearest/extend_until_reachable + `load_freightnet` build-if-missing); **`geo_select`** (`src/components/geo_select.py` — per-door candidate selection: k-NN seeds + detour-corridor ellipse φ + exhaustive bidirectional flight-frontier H_max; confirmed NOT a dominance prune); build-time **per-HAWB fallback** (`FallbackPolicy`/`_max_path_cost`/`air_leg_cost_ub` — longest UB-path × 1.5, trivial-when-stranded, arrives at T^abs = max tardiness); **FULL retirement of the hardcoded-region candidate path** (generation is door-only; nominal gateway via FreightNet; candidates + air-arc restriction resolve at build via `build_geo_air_graph`; persistence dropped candidate JSON + persisted `fallback_cost`); and the **D-F6 v2** methodology change (Δ_k = ready + base_transit(lane) + sla_offset(tier) — pre-committed tier×lane SLA, graph-free; `committed_deadline`; sla_offset 12/40/120→12/24/48; `model/precommitted_sla_deadline_proposal.md` APPROVED). **Long deep-dive sequence** with the user on each (fallback economics, $1-vs-dominating, the Δ_k circularity). **B5 re-measured then REOPENED:** the geo corridor cut LP size (103→~37 arcs) but a **4-agent critique (`docs/critique/17`)** found solve time is seed-dependent and unbounded — n=15/d7 = 23s/132s/>5min-unfinished over seeds 0/1/2; "21.2s RESOLVED" was a lucky seed. Critique also found **BLK-2: `air_leg_cost_ub` under-bounds BSA per_flight** (volume-bound cargo) → fallback can strand a routable HAWB. Both BLOCKING logged for user review tomorrow; nothing fixed in response yet. Memory to add S38: D-F6 v2 + B5-reopened. Prior:) 2026-06-14 (Session 36 — **F1 Slice A + B1–B4 BUILT; standing review agents run; B5 found region→region intractable at the proof cell.** 205 passed, ruff clean. Slice A = independent network-supply draw (κ+α integer multinomial on a new `supply` RNG stream, CRN-gated). Ran the 3 standing review agents (`docs/critique/14/15/16`): seam audit no-BLOCKING + validated Slice A; **red-team R1/R2 DEFLATED after user pushback** (loose-corner gate "no-op M₁ passes it" = strawman in a deterministic 2-solver sim; "average hides a lottery" = the real value distribution + correct estimator) → logged as minor scorer-build notes, §13 NOT reopened; drifts fixed (fallback 2.0→1.5; leg-count flagged). Slice B B1–B4 = **region→region routing (D-A24)**: additive `Hawb.origin/dest_candidates` (single-airport default = byte-identical), trucking "diamond" ground chains, **per-tail Δ^post N7 fix** (was a flat subgraph sum → double-counted multi-POD), dispatch-lead per candidate origin, `check_consolidation_coherence` invariant, distance-based (haversine) trucking-matrix generator + **SFO** 3rd dest gateway, `DEMAND_LANES` retired. Fixed latent hash-order determinism bugs region→region exposed (sets/frozensets feeding HiGHS sums + actuals presampler) + persistence round-trip (candidate JSON columns). **B5 = OPEN BLOCKER:** n=15/days=7 >5min; **720→168h backstop (now `config/forwarder_graph_config.json`, forwarder-level) did NOT fix it** — the real fix is geographic graph-gen / the network layer (see RESUME). User rejected cost-based dominance pruning (strands HAWBs under tight supply). Memories: `project_graph_generation_vision`, `feedback_no_standalone_cost_pruning`. Prior:) 2026-06-13 (Session 35 — **F1 REDESIGN → `arrival_only_replan_methodology.md` §13 APPROVED v4 (independent network-supply model + region→region routing).** No code touched; 193 passed (unchanged). User asked for a critique round before building the planned F1 ("continuous κ"); it turned into a ground-up redesign driven by user pushback. **Key reversal:** the original F1 (weight-only kg `cap_a = peak_demand/κ` on `BsaContract.cap`) was found (a) unable to ration volume-limited cargo — all cargo is 120–240 kg/cbm, below the LD3 333 break-even, so C.5b-volume binds first and the kg dial is a near-no-op; and (b) **circular — it constructed supply from the demand just generated.** Redesign (4 critique rounds, §13 v1→v4): **supply is generated INDEPENDENTLY of demand across the network** — per-flight **integer ULD positions** from a new `supply` RNG stream; **κ** = network tightness (`total_N=round(E[Σ SE_k]/κ)`, analytic no-consolidation slot mean — no demand coupling); **α** = Dirichlet concentration (lumpiness); per-lane tightness EMERGES from random supply vs where demand lands; the mismatch IS the problem. **D-A24 LOCKED: region→region routing** (origin/dest airport + lane + flight are optimizer decisions via a per-airport trucking matrix + multi-O/D subgraphs; fixed-lane `DEMAND_LANES` retired — scope expansion, committed). Integer ULDs **dissolve the volume defect** — the existing 2D **C.5/C.5b** is the contracted gate as-is (D-A20/21 WITHDRAWN; cap_a/SE/peak_demand machinery gone). Three supply sources/flight: contracted (C.5b) / spot (NEW per-arc `Σ cw_k·x ≤ cap^spot`, two-sided price) / fallback (1.5× graph-derived worst-spot-route, amends D-A12). **Falsifiability:** dedicated negative-control retired → loose corner of the (κ,α,λ) sweep gated `|L2|<CI`; regret floor demoted to a self-check; M₀ (D-A23) = competent single-pass baseline (deterministic within-cycle consolidation). Memory added: `project_supply_independent_of_demand`. **▶ RESUME = start F1 build against §13 v4.** Prior:) 2026-06-11 (Session 34 — **critique-12 clarity rewrite + 7-agent integration/framework review (`docs/critique/13` + `docs/design/e2e_test_plan.md`) + Wave-0/1/2-partial code fixes.** 193 passed, ruff clean throughout. Rewrote `docs/critique/12` § Numeric walkthroughs (F1/F2/F3) clearer (shared κ×λ picture + ASCII timeline). Ran a **7-agent review** (code-arch / seams / graph-gen / model-realism / sim-realism / numerics / test-design) → new findings beyond F1–F8: **N1** $1M fallback wrecks the relative MIP gap (L2 is a *difference* of two such objectives); **N2** PYTHONHASHSEED documented-but-unset; **N3** no state-owner for sim-clock/ledger/RNG before 2c; **N4/N5/N6** the arrival-only headline excludes the thesis's primary driver (disruption) + one `[CAL]` knob sets both L2 mechanism and `cw_flex` denominator + π_hind floor near-vacuous; **N7–N18** latent correctness (Δ^post subgraph-sum, A_k^min over unfiltered graph, air-arc board-by CO* not STD, dispatch over-applied, carrier deny-layer missing, ULD 8.0-vs-4.5 mismatch). Fixed **Wave 0** (N1 `compute_fallback_cost`≈$40k / N2 cross-process determinism proof-test / F8 CRN / N17 asserts→raises / n1 aux bound / n2 deadline guard / N7 single-tail guard / N18 dedupe) and **Wave 1** (N9 board-by min(CO*,STD) / N10 dispatch→origin-POL / N8 A_k^min over admissible arcs / N12 ULD vol 8→4.5; N11 deferred). **Wave 2 STARTED** (the critique-12 fold; all `[CAL]` locked: L_cut=6h, κ grid {0.5,0.8,1,1.25,1.5,2}, roll $50, F4 ~60/40, **τ=1.5%** from researched forwarder economics): **F2a cutoffs** (`dep−6h`, `_SCHED_OFFSET_H=10`, HKG bank +8) + **F2b binding-leg anchor** (`_contracted_by_dest_day`, `air_graph.latest_ready` clamp = no born-dead) DONE. **▶ RESUME = build F1 (continuous κ = demand/slots).** Prior:) 2026-06-09 (Session 32 — **generator-to-files BUILT** (`write_scenario` persists a full scenario + pre-samples ALL leg/component actuals, deterministic `s=0`; G1–G4 folded; 147 passed, ruff clean). **Arrival-only methodology APPROVED + reconciled** (`model/arrival_only_replan_methodology.md` v0.1 — air transit DETERMINISTIC; per-leg Gaussian jitter AND a discrete disruption-stream BOTH rejected as "fake" manufactured failure; the ONLY stochastic process is demand arrival; predicate-9/z_tier/σ̂ retired for air → deterministic `A ≤ Δ_k`; M₀ incremental-greedy vs M₁ open-book re-opt; disruption recourse = a TESTED capability not a value source; L2 = conservative lower bound) → `air_transit_time.md` v0.3 + `backtest_methodology.md` v0.5; code aligned (deterministic pre-sampling). **Arrival-process shape LOCKED** (§10, D-A5..D-A8: daily departures `D≈7`; `known_at = cutoff(d*) − B`; tier-coupled book-lead [EXPRESS late+tight, DEFERRED early+slack]; fixed `N`; sweep κ×λ). **4-agent simulation design review** (`docs/critique/11`) → hardening folded as §12 / **D-A9..D-A16**: D-A9 independent-arrival HEADLINE (coupled-favorable = upper bracket; removes "built the sim to win"); D-A10 pre-registered null + required negative-control cell; D-A11 M₀=pin-prior-soft + the `M₁'` control arm; D-A12 realized_cost excludes the C.10 penalty + reshuffle-gated headline + retire $1M fallback; D-A13 one time-scalar source of truth; D-A14 batch-cutoff H₀ headline; D-A15 lower-bound scoped to transit-reliability only; D-A16 `cap_a`/`A_c` frozen. **▶ NEXT: λ arrival-stream generator + 2-FLEX** (D-A9 independent-arrival headline; 2c blocked on M₀ soft-pin + `M₁'`). Prior:) 2026-06-08 (Session 31 — **input-layer fork = Option A**; input layer + 5 hardening edits built on `scenario_db.py`; **5-agent proactive review** folded (`docs/critique/10`); **round-trip spike BUILT & GREEN** (`data/synthetic/scenario_io.py`) → schema validated lossless (caught the flight_id-not-unique bug); `data_model.md` reconciled; ERD written; one-offer-one-rate (memory `reference_air_offer_rate_cardinality`); decisions D1 capacity ULD-slot-only / D2 `mct_h` / D3 `{offer_id}:{uld_type}` / D4 delete `contracted_rate_per_kg`. 139 passed, ruff clean. **▶ NEXT: generator-to-files (G1–G4) → 2-FLEX → 2c → arms → Stage 3.** Prior: Session 29 — **Stage 2a synthetic generator + 2b transit-time BUILT & isolation-tested** (110 passed, ruff clean); **OTP-control reframe** locked (OTP = population-over-time binary metric, controlled at graph-gen tier-reliability filter [new air-model predicate 9] + per-shipment penalty, NOT chance constraints; one-draw-per-leg realization, no per-route MC); **specs APPROVED:** `air_transit_time.md` v0.2 (2b), `backtest_methodology.md` v0.4, `flexibility_model.md` v0.3 (2-FLEX), `scenario_io_and_replay.md` v0.2 (SQLite-per-scenario, generate-all-first→deterministic replay, route-versioning plan history); **route-ordering code fix** (`air_graph.order_route`); **6 multi-agent review rounds** folded (OTP reframe ×2, 2-FLEX ×2, proof-wide ×3 [calibration/seam/red-team], scenario-IO ×1). Memories added: `project_otp_control_reframe`, `reference_air_spot_contract_ratio`, `feedback_session_log_last_entry`. **▶ NEXT BUILD: `scenario_db` module (schema-first), then generator-to-files → 2-FLEX → replay loop.** Session 28 follows:) 2026-06-06 (Session 28 — **backtest methodology APPROVED (v0.3)** + reconciled `docs/design/remaining-execution-plan.md` to v0.3 + ran critique agents on both; reconciled `product_thesis.md §2`; **created `BUILD_STATUS.md`** as the canonical built/remaining tracker — now read first at session start + refreshed fully every sign-off, per CLAUDE.md; demand-arrival micro-spike `spikes/stage0_5b_demand_arrival_replan.py` PASSED. Air optimizer M1–M6 complete. **▶ NEXT BUILD: Stage 2a synthetic data generator.** Session 27 context follows:) 2026-06-04 (Session 27 — strategy + planning thread, **no air-build code changed**: created `product_thesis.md` (four-layer value gradient L1 planner→L2 replan→L3 capacity controller→L4 market intel; moat = data flywheel; replan-savings estimation method) + `docs/design/remaining-execution-plan.md` (air-deep-to-thesis-proof bet, revised v0.2 after 3-agent critique). **DECISIONS LOCKED: D1 = Opt 1 (BSA contract entity, builds per_flight + equalized in M4, M4b folded in); D2 = Opt A (base `schedule_legs` + per-mode detail tables; air 2D capacity first-class); route-versioning model (full immutable snapshots, resolution×firmness orthogonal, `supply_ref` ∈ {schedule_leg, ondemand_arc, rate_card_lane, NULL}).** M4 now UNBLOCKED; next is data_model.md spec (option b — sketch then write) → Stage 0.5 fail-fast spike → M4 build. Session 26 context follows:) 2026-06-03 (Session 26 — side thread, **no air-build code changed**: LOC accounting + phased burndown (current ~3,233 real LOC; full commercial product est. ~112K); created `synthetic_data_contract.md` (NEW, repo root) — supply/demand generator output contract vs `data_model.md`. Decisions locked: add `schedule_legs`; pre-generate + sim-clock-view reveal; `external_id` idempotency; orchestrator-driven capacity decrement; defined `demand_generator_configs`. **OPEN:** schedule-schema *structure* (base+per-mode-detail [rec] / separate-per-mode / single-wide) — blocks promoting `schedule_legs` + `demand_generator_configs` into `data_model.md`. Air MILP M4 schema decision remains the PRIMARY next action. See "Parallel thread (Session 26)" below. — Session 25 context:) 2026-06-02 (Session 25 — Air MILP slices **M2** (C.4 chargeable-weight density mixing + flat_rate bucket cost + C.5c cap) and **M3** (min_flat_breaks IATA next-break-down round-up) landed; consolidation now pays; all three non-BSA rate families billed; billing validation reworked to recompute CW from realized routing + assert each family's closed form. Full suite 84 passed, ruff clean. Ended on the **M4 schema decision** — three options (contract entity / flat+tag / per_flight-only) with numeric walkthroughs + a pivot-vs-allowance clarification captured in `docs/design/air_milp_m4_bsa_schema_options.md`; **DECISION OPEN, user deep-dives next session before any M4 code.** Prior context below from Session 23 —)
...
## RESUME HERE (next session — Session 55)  [supersedes the S52/S53 blocks below]

**▶ THE STATE IN ONE LINE:** the S52 "reserve-space-early / assign-cargo-late" fix is now a **fully
designed, 7-agent-verified, approved-for-build 2D spot-reservation model** (replaces locked D-A1), and
**slice S1 (schema split) is BUILT + committed, 386 green, byte-identical.** Resume by building **slice
S2** (`ReplayState._reserved_spot` + post-solve usage ratchet — behavior-neutral until S3), then S3–S7.

**READ FIRST:** spec `docs/design/spot_reservation_2d_design_s54.md` (REV 3 — the build follows it slice
by slice) + memory `project_s54_2d_spot_reservation` (resume anchor: design + decisions + slice state).
The 7 slices + which tests each carries are in `BUILD_STATUS.md` "Near-term critical path". **S4 (2D MILP
caps — headline volume-breach test) and S6 (sunk-cost objective + h0 binding-dim + tie-break) are the
delicate ones.** Tardiness ON in every test; real HiGHS.

**Locked S54 decisions:** reserve-early/assign-late; **sunk-cost/free-reserved-capacity** cost (reserved
space free at margin ⇒ optimizer fills, no L2 artifact; total = `family_cost(peak reserved)`);
`C^v=C^w/333.33` honest ~2× tighter volume → recalibrate operating point via τ (S7); `penalty_frac=1`
(NC-a — monotone double-charge on service reroutes — dismissed as an air-domain non-issue); S51 M1′-dump
reversal blessed (dump was an artifact of unheld space, not a real cost of rigidity); chain
guaranteed→empirical. **Cancellation (`penalty_frac<1`, releasable) = v2, tracked in Deferred.**

**S52/S53 blocks below are HISTORICAL** — the 5 S52 decisions were resolved this session (the redesign IS
their resolution). Do not re-open them.

**The 5 decisions to get signed off, then ONE consistent doc pass (grounding doc + memory + proposal .tex):**
(1) D-A1 amendment (split tender→reserve+assign); (2) **friction model** — cutoff-gated *fractional* no-show
penalty (magnitude?) + reserve **monotone vs releasable-before-cutoff** (the load-bearing knob); (3) estimator
(D5-separated + per-tier kg + 3-cause); (4) M1′ symmetric-reserve recoding (behavior change to built arm);
(5) methodology framing (general-cargo scope; reserve = master-slot quantity; special-commodity/security
exceptions). **THEN, only after sign-off: build** (start by making M1′ coherent) — no code yet.

**What the diagnostics proved (cell τ=3.0/seed=3/n=70, all read-only):** every HAWB has feasible in-time routes
(timing WALL ruled out); the clairvoyant perfect-info solve gets 9% fallback / 60% OTP (engine works, instance
ceiling ~60% OTP at 3× supply); **no-decay staged M1 ≈ clairvoyant (7% fb / 63% OTP) ⇒ 100% of the staged-arm
degradation is the DECAY, staging costs ~nothing.** BUT the decay itself is grounded and fine — the actual bug
is that **cargo arrives ~2 days before its cutoff (median book-lead 1.9d) and only commits AT the cutoff, so it
never catches spot** (which fills over ~2 weeks; at commit φ≈15%, at ~5 days out φ≈56%). `L2_real%` is a mirage
under Model-Y (M1' "cheap" only by dumping 57% to fallback) — retire it; headline = service + total cost.

**▶ S52 RESOLVED the design (was an open discussion):** the split-`tender` mechanism is now a written
proposal + grounded. Reserve-space-early/assign-cargo-late is CONFIRMED real for spot (not just BSA) —
the master booking is cargo-agnostic (route/commodity/weight + own AWB #). Friction refined to cutoff-gated
fractional no-show (not flat full charge). 3 capacity tiers (hard/soft BSA + spot) + fallback. All captured
in memory `project_s52_reserve_assign_synthesis`. **The next step after sign-off is the sweep** — re-anchor
`scripts/d3_sweep.py` τ ladder (stale `{0.75,1.05}`) + metric reframe, run it = the thesis number.

**Known, lower-priority:** contracted (firm) Asia→US capacity is only **0.89× demand** + metro-mismatched
(Midwest 59% of demand but 0.8× at cutoff; East 2.0× surplus); half the spot is oversized feeder. Reconcile the
48h soft-BSA cliff vs the "~2-week allotment release" the booking agents cited.

**Open calibration items (none block the build):** `reposition_rho` value (MRN — sweep meanwhile),
origin/dest tightness band widths (INFERRED — calibrate to BAI30/BAI80 dispersion), q^O China-dominant
share (MRN — currently equal thirds), per-gateway transit hours (grounded), final metro membership.

**Uncommitted-at-S50-start note (now committed at S50 sign-off):** the amendment + design doc + slice-1/2/3
code + tests. The water-bug fix was committed mid-session (`7c08539`). Scratch S50 probes
(`scripts/_fb_cause_probe.py`, `_decay_ab_probe.py`, `_supply_balance_probe.py`, `_*.err`/`.out`) left
uncommitted; `scripts/d3_sweep.py` is a keeper (committed).

**Built so far (region path, `data/synthetic/air_generator.py`, gated by `GenConfig.tau is not None`):**
`_expected_cw_mean` · `_size_total_supply` · `_dest_region_shares` (q_R) · `_draw_region_tightness` (τ_R) ·
`_size_region_supply` (S_R) · `_spread_positions` + `_draw_region_network_supply` · `_split_contracted`
(soft `per_flight` + hard `equalized`, short-region-first) · `_build_region_rate_catalog` (spot from S_R,
contracted $4.2 < spot $5.5). `GenConfig` gained `tau`/`region_mix`/`contracted_share`/`hard_bsa_frac`; new
`region_tightness` RNG stream. **Sanity-checked** (τ=0.9, n=120): LAX τ=0.72 short / ORD 0.92 bal / SFO
1.25 slack, hard BSA on LAX. CRN gate + build+solve both green.

**Resume work, one slice at a time, isolation-tested:**
- **1c — arrival spread.** Replace the binary `tier_coupled_arrival` flag with the per-tier lead-time
  **bucket mixture** (3×4 weight matrix, within-tier EXPRESS late tail, fixed-draw-count CRN) in
  `_gen_arrivals`. Add opt-in `lead_buckets` to `ArrivalConfig`. **Open scoping Q:** does
  `generate_arrival_scenario` also need the region/τ supply path wired (it still uses the κ path)? Likely
  a small wiring step — decide at 1c start.
- **D2 — schedule substrate.** Tag flights `deck ∈ {belly, freighter}` in `tpeb_air_instance` (belly =
  thinner spot caps); one deferred-air slow-discount arc per lane. No new MILP constraint.

**Then:** Phase 2 (productionize the corrected `cap_decay` in `run_replay`: linear φ + per-flight jitter;
only free pool decays; per-arm/per-cycle firm floor; tests no-cascade/conservation/determinism) → Phase 3
(scorer: `real_cost`+`fallback_penalty` kept SEPARATE; both OTP forms; savings pair `L2_real%` off M₁',
`Total_real%` off H₀, `L2_pen$` separate; fallback 3-cause split; metrics-schema decision) → Phase 4
(calibrate + D3 factorial sweep = thesis number; **re-confirm C2 tractability first** — the compute-heavy
phase). **Locked decisions (S47, §14.6):** D1 flat+decay (block→v2) · D2 build belly/freighter+deferred ·
D3 sweep both settings · D4 both OTP forms · D5 two cost numbers separate (ψ_cap RETIRED). Governing:
`arrival_only_replan_methodology.md` §14. **Deferred to v2:** increasing-block spot (D1), charter, spot
regime-inversion (S3), density-floor widening (S4).

**What S46 settled.** The parked Stage-3 fork was superseded: the user chose to redesign the whole air
sim (all capacity types + a single supply/demand tightness dial `τ` + measure cost/OTP/fallback). A
**7-agent design fan-out** (`docs/design/s46_capacity_redesign/` 01–06) produced the architecture; the
**red-team (05) flagged 2 FATAL risks** that the design could reproduce S45 (`L2_capacity≈0`); the
**user's temporal insight (06)** — time-decaying capacity + within-tier arrival spread — became the
binding mechanism. A **kill-shot** (`scripts/killshot_s46.py` + a minimal default-off `cap_decay` hook
on `run_replay`) **PASSED across 5 seeds, W on and off**: the two replan arms route **different
chargeable-kg per capacity tier** (the scalar S45 never computed, where it was $0) ⇒ **capacity binds**;
open-book M1 dominates on cost / fallback / tardiness-severity. Suite **311 green, replay.py ruff-clean**.

**The decay model was CORRECTED with the user (load-bearing):** decaying soft-BSA was a MODELING ERROR —
a soft-BSA is a RESERVED block, a used ULD is committed (paid via pivot) and does not shrink; **only the
FREE pool decays**, and used ULDs must be counted PER commodity-group. Corrected hook: spot
`avail=booked+(nom−booked)·φ`; soft-BSA `avail=used_ULDs+round((N−used_ULDs)·φ)`; hard-BSA reserved.
Verified feasible (70/70) all 5 seeds decay-on, W-on.

**The build plan (record):** **Phase 0** methodology gate (approval-required) → **Phase 1** generator
(1a τ_ℓ + short/bal/slack · 1b composition + hard-BSA generation + rate re-anchor below spot · 1c
arrival spread · 1d *[v2]* increasing-block spot + belly/deferred topology) → **Phase 2** decay in the
real planner (productionize the corrected hook) → **Phase 3** scorer/metrics (3 families + grace-band
OTP + sounder L2 capacity/consolidation attribution) → **Phase 4** calibrate + sweep = the thesis number
→ **Phase 5** tractability (de-risked: per-cycle decomposition already solves C2). **Decisions to bake
into Phase 0:** D1 spot flat-finite+decay vs increasing-block (rec FLAT) · D2 belly+deferred = v2 · D3
realism knobs (composition ~0.55 contracted / `hard_bsa_frac` ~0.35 / τ base 1.0–1.15, τ≤0.8 stress arm)
· D4 OTP metric (rec: report all 3 delivery states, lead with fallback%+p95, grace-band secondary) · D5
better L2 attribution. **Recommended LEAN path** (defer 1d).

**Caveats to carry:** the kill-shot is SCRATCH (monkeypatched hook, hardcoded rates) — Phase 0/1 rebuild
it properly. OTP is strict-binary (`arrival ≤ Δ_k`); fallback% / tardiness-p95 are the truer service
metrics (D4). Realism (02) BLOCKING: τ<1 network-wide is PEAK not normal; 0.70 contracted too high for
mid-market. L2 capacity-vs-consolidation split is currently approximate (per-tier×price).

**S46 artifacts (all in `docs/design/s46_capacity_redesign/`):** `01_architecture` / `02_realism` /
`03_metrics` / `04_tractability` / `05_redteam` / `06_temporal` (+ locked-decisions block). Proof:
`scripts/killshot_s46.py`. Prior S45 artifacts (`l2_decomposition_s45` etc.) still in `docs/design/`.

---

**(S44/S45-prior — historical; 2c-7 partly built, now downstream of the Stage-3 fork above.)**

**Where we are:** 2c-7 disruption recourse is partly built. Slice 1 (injection + §6 gate, absorbable no-op)
and slice 2a (recovery mechanism — `_disrupted_sim` + departed-clock `_flown_prefix` + prefix-pin re-solve +
recourse tardiness weights) are DONE and green (311 passed, ruff clean). The recourse penalty is calibrated
(W=1.3/0.65/0.32 by tier; reuses the existing C.10 quadratic PWL). The methodology now has **§6.1** (mid-shipment
recourse anchoring + node-anchored fallback, for sim AND the production planner). Ledger is still audit-only.

**▶ FIRST: step 1b — the node-anchored fallback** (methodology §6.1). Bigger than first scoped; three coupled
pieces: (i) **graph injection** — a dynamic FALLBACK arc from the anchor node (head of the last departed arc) →
dest door, arrives `T^abs`, cost `1.5×longest-UB(anchor→dest)` (`_max_path_cost`+`air_leg_cost_ub`) or `trivial=1.0`
if no real path, ADDED to the sunk prefix; resolve dynamic arcs alongside `ag_orig`. (ii) **Gate fix** — a
NON-flagged shipment whose pinned flight is PRUNED in `sim_t` (a cancel, or a delay past T^abs) must also be
flagged/unlocked, else its stale pin makes the solve INFEASIBLE (the cancel path already flags via `new_break`;
delay-past-T^abs does not — `route_eta` reads it as "rides late"). (iii) **A multi-arc test bed** — seed-3 M1
routes are single-leg or single interline ARCS, so a true in-transit strand (cargo at an intermediate hub, no
onward) can't be constructed there; build a small multi-arc scenario / hand-construct a graph. Removes the
`NotImplementedError("...slice 3...")` raise (recovery always feasible).

**Then 2b — ledger conservation through the unlock** (the thing 2c-7 most exists to prove): populate real
`tendered` (currently hardcoded 0 / audit-only), make `ReplayState.release()` return the slot to free, relax the
`reconcile` tendered-monotonicity guard to permit a decrease ONLY via an explicit release, and assert conservation
through the unlock. The existing `tendered==0` audit test (`test_ledger_conserves...`) flips to a conservation
assertion. Then the **interline-internal break** (leg-level prefix split — cargo stranded mid-arc).

**Then, in order:** re-measure the **MFBlink lift across more (κ,α,λ) cells** (BLK-1c caveat below) →
**Stage 3: the (κ,α,λ) sweep** + L1/L2 decomposition with `|L2|<CI` loose-corner gate = **the thesis number**
(findings 1–2 land here). **Standing review agents** (calibration / seam / red-team) — last run S36, due S43,
STILL NOT RUN; offer early in S45.

**Reconcile at leisure:** (1) methodology **D-A14 "batch-cutoff H₀"** is now daily-batch — update the doc + the
`project_*` memory. (2) **The $40k fallback figure (critique-13 N1) is STALE** — live `FallbackPolicy` ≈ $16-18k;
fix wherever it's quoted. (3) **PDF still 3 compiles behind** (S38+S39+S41) — user owns the compile.

**2c-6 DONE (S43, committed this sign-off).** `run_replay(conn, scenario_dir, *, seed, arm,
execution_id=None, params=None, cycle_times=None)` now dispatches on `arm ∈ {M0, M1p, M1, H0, PIH}`.
**H₀** = `_plan_cycle_h0` (batch-at-cutoff; cheapest-standalone singleton-solve routing; priors frozen;
all-pinned billing solve; fallback-roll on capacity contention). **π_hind** = `PIH` (single full-book
no-pin cycle). `score_run` scores both unchanged (`metrics.l1` still NULL — Stage 3). The 2c-5 scorer +
2c-3/2c-4 machinery are unchanged. The ledger is still **audit-only** (`tendered=0`) until 2c-7 wires the
split. M₁'/H₀ realized arrival uses `route_reliability` (the deterministic Â).

**⚠ BLK-1c instance-specificity caveat (before fully locking it closed).** The MFBlink lift is confirmed on
**seed-2 + seed-0 only**, and is κ-invariant *on this monolithic LP* (the MFB structure sits on κ-independent
spot/flat lanes). Before Stage 3, **re-measure the lift across more (κ,α,λ) cells/seeds** with
`scripts/lp_gap_probe.py` (root gap + per-family fractionality) and `scripts/mfb_lab.py` (`link` variant + `--mip`
validity). If a future cell shows a large residual gap, the next lever is the multiple-choice / DCC MFB
reformulation (scaffolded as `mfb_lab.py` variant `mc`) or the parked 44% Pareto-dominance candidate cut
(recomputed vs **remaining ledger capacity** in replay, never nominal cap). Other BLK-1c tooling unchanged:
`scripts/seed2_repro.py`, `scripts/probe_tractability.py`.

**S41 build state (280 passed, ruff clean src/tests — committed `27ea807` + this sign-off's trackers).** MFBlink
cut in `_build_min_flat_breaks_cost`. **Billing validator now gap-aware:** `_check_billing` gates over-billing on
the actual `mip_gap` (exact ⟺ ≤1e-9), not the `OPTIMAL` status string — under-billing still always raises;
`mip_gap` threaded through `_validate_billing`/`_validate_bsa`. **C.10 tardiness build skipped when all W_k=0**
(full build retained when any W_k>0). tex reconciled (MFBlink + numeric example §lin-bucket; gap-aware validator
note §objective; summary-table + code-annotation). Commit also folded in the previously-uncommitted **S40 code**
(`MilpParams.__post_init__` c_fix>0 floor + C.2b-drop + 2 tests). **PDF 3 compiles behind (S38+S39+S41) — user owns
recompile.** 6 scratch scripts (`lp_gap_probe`, `mfb_lab`, `cut_lab`, `cut_introspect`, `frac_diag`,
`pareto_ceiling`) untracked per user. Governing: `arrival_only_replan_methodology.md` (§13 v4 + S38 Reading-B),
`backtest_methodology.md`, `precommitted_sla_deadline_proposal.md`.

**▶ Standing review agents next due S43** (calibration / interface-seam / backtest red-team; last run S36).
