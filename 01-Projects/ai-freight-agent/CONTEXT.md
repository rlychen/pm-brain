...
## RESUME HERE (next session — Session 36)

**▶ FIRST ACTION: start the F1 build against `arrival_only_replan_methodology.md` §13 v4 (APPROVED S35).** The
original F1 ("continuous κ = peak_demand/κ on `BsaContract.cap`") is **SUPERSEDED** — read §13 v4 (the whole
section, esp. D-A18 rev, D-A19 rev, D-A20/21 WITHDRAWN, D-A23, **D-A24 region→region LOCKED**, D-A10 rev) before
writing code. The Wave-2 `[CAL]` knobs (κ grid, F4, τ) from S34 are also superseded by the network-supply model.

**§13 v4 governing design (what F1 builds):**
- **Supply independent of demand.** Per-flight contracted = **integer ULD positions** from a NEW `supply` RNG
  sub-stream (add `"supply"` to `scenario_db.RNG_STREAMS`; today `n_uld` rides the `rates` stream — move it).
  **κ** = network tightness: `total_N = round(E[Σ SE_k]/κ)`, `E[Σ SE_k]` = analytic mean of standalone slot use
  `max(w/1500, v/4.5)` over the density-mix distribution (closed-form, reads ZERO demand draws → CRN). **α** =
  Dirichlet concentration: per-flight counts ~ `Multinomial(total_N, Dirichlet(α))`. κ is a coarse integer ladder
  at proof scale.
- **D-A24 region→region (LOCKED).** Origin/dest airport + lane + flight = optimizer decisions. Build: (1) per-airport
  trucking-cost matrix on the HAWB (replaces scalar `pickup_cost`/`delivery_cost`); (2) **multi-O/D subgraph
  construction** — each HAWB's subgraph spans all candidate airport pairs in its regions, arc IDs airport-pair-specific;
  (3) tractability re-check. Fixed-lane `DEMAND_LANES` retired.
- **Contracted gate = existing 2D C.5/C.5b** (`Σw≤1500η`, `Σv≤4.5η`, `η≤N_f`); just feed the drawn integer `N_f`,
  replacing `n_uld=max(1,round(2·scale))`. Assert `BsaContract.cap=={}`. CW stays in billing only.
- **Spot** = NEW per-arc constraint `Σ_k cw_k·x_{k,a} ≤ cap^spot_a` (NOT C.5c; sums cw_k over riders, works for coload
  + flat), price = base × m (m ~ two-sided band, supply stream). **Fallback** = unlimited at
  `1.5·[top_spot_rate·CW_k·max_air_legs(graph-derived) + ground chain]` (dominant, amends D-A12).
- **Falsifiability:** loose corner of (κ,α,λ) sweep gated `|L2|<CI`; regret floor = self-check only. **M₀** =
  competent single-pass baseline (optimal within-cycle consolidation, deterministic `(tender_at,tier,id)` tie-break,
  pins priors). Report fraction of L2=0 draws + per-airport binding-rate + post-consolidation occupancy.

**Suggested first slice (fail-fast):** the independent network-supply draw (κ+α integer multinomial on the new
`supply` stream) + the `RNG_STREAMS` edit + the **CRN test** (vary κ/α ⇒ demand draw byte-identical) — validate the
supply mechanic BEFORE the heavier region→region graph work. Then region→region subgraphs + trucking matrix, then the
spot/fallback MILP changes. Tests per §13 "Build implication for F1."

**Suite still 193 passed (no code changed S35).** Memory: `project_supply_independent_of_demand`. Standing review
agents (calibration/seam/red-team) last full run S29 — **next due ~S36**.
