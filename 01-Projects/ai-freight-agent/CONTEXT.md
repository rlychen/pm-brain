...
## RESUME HERE (next session — Session 37)

**▶ FIRST ACTION: build `FreightNet` — the NETWORK LAYER (prerequisite for the real graph-gen, user-requested S36).**
This comes BEFORE any graph-generation logic. **Name LOCKED (S36): `FreightNet`** — both the data model AND the
service (e.g. "all nodes within X distance of a point"). Scope: **DB tables of every physical freight node** — types
airport / ocean port / rail terminal / ICD-dry-port / CFS-warehouse / trucking terminal / border-crossing /
bonded-FTZ / barge terminal; per-node fields = id, type, name, codes (IATA/ICAO/UN-LOCODE), **full address,
city, admin region, country, lat/lon**, timezone, modal capabilities. Real ref data (UN/LOCODE, IATA — free).
→ memory `project_graph_generation_vision`.

**▶ THEN: graph-gen review (Q&A, sim vs real) — this also FIXES the B5 tractability blocker.** Today a HAWB's
subgraph = **hardcoded candidate-airport set** (`_ORIGIN_REGION`/`_DEST_REGION` in `air_generator.py`) +
algorithmic feasibility prune (`air_graph.build_hawb_subgraph`); topology = the TPEB fixture. The real design =
**geographic on-the-fly propagation** over the network layer (origin→airports within X km, dest→within Y km,
extend radius until reachable; flight-propagate both frontiers until connected). That replaces "all 9 airport
pairs for every HAWB" — the root of B5's intractability — with the few geographically-near ones. **Safe
(distance-based), NOT the rejected cost-based dominance pruning** (memory `feedback_no_standalone_cost_pruning`:
under tight supply the expensive pair is the feasible fallback → cost pruning strands HAWBs).

**▶ THEN: F1 Slice C** — spot per-arc CW-cap `Σ cw_k·x ≤ cap^spot` + two-sided spot price (base×m) + route-based
fallback (`1.5× graph-derived worst-spot-route`), per §13/D-A19. Then Wave-3 N3 `ReplayState` owner → 2c replay
loop + arms (M₀/M₁/M₁'/π_hind) → scorer + (κ,α,λ) sweep = the proof.

**S36 build state (205 passed, ruff clean):** F1 **Slice A DONE** (independent network-supply κ+α multinomial on
the `supply` RNG stream; CRN-gated). Standing review agents RUN (`docs/critique/14/15/16`; R1/R2 deflated, drifts
fixed). F1 **Slice B B1–B4 DONE** (region→region: additive `Hawb` `origin/dest_candidates`; per-tail Δ^post N7
fix; dispatch-per-candidate; `check_consolidation_coherence`; distance-based trucking generator + SFO topology +
`config/forwarder_graph_config.json` backstop). **B5 = the open blocker**: region→region intractable at proof cell
(n=15/days=7 >5min); backstop 720→168h did NOT fix it (graph size unchanged); the geographic graph-gen above is
the real fix. Governing methodology still `model/arrival_only_replan_methodology.md` §13 v4.
