...
## RESUME HERE (next session — Session 39)

**▶ FIRST ACTION: continue 2c (the replay-loop proof machinery). Both S37 BLOCKING are closed; critique-17
is fully triaged.** Next slice is **2c-2 — populate `tender_at`** (currently UNSET in the generator/persistence;
the loop's tender step has no data). Per D-A1, `tender_at` = the HAWB's binding cutoff. Wire it in
`air_generator` (`write_arrival_scenario`) + `scenario_io` persistence + the `shipments.tender_at` column.

**▶ THEN 2c-3 — replay-loop skeleton (M₁ open-book arm).** New `run_replay(scenario, arm)` on top of
`ReplayState` (`src/replay.py`): walk the clock at the config cadence → `state.visible()` → `solve` (pin the
tendered set via `state.tendered_set()`) → extract per-arc allocation (contracted η from
`AirSolution.mawb_uld_counts`; spot Σcw over riders) → `state.reconcile(arc, tendered, committed_untendered)`
→ tender HAWBs with `tender_at ≤ clock` (`mark_tendered` + the ledger tender). Record planning_run/route
snapshots (§7). NO scorer yet.

**▶ THEN, in order:** 2c-4 M₀ (greedy) + M₁′ (single-pass, pin priors via the 2c-1 `pinned` param) → 2c-5
**scorer** (deterministic replay of `air_transit_time §4` with the FROZEN actuals → realized arrival/OTP/cost/
fallback; NOT a naive leg-sum) → 2c-6 H₀ heuristic + π_hind (degenerate single-cycle, no tender lock) → 2c-7
recourse fixtures (methodology §6, the 3 disruption fixtures) → **Stage 3: the (κ,α,λ) sweep** + L1/L2
decomposition with `|L2|<CI` loose-corner gate = **the thesis number.**

**S38 build state (278 passed, ruff clean — committed at S38 sign-off).** Reading-B decomposition locked
(M₀ greedy / M₁′ single-pass / M₁ open-book; L1=H₀−M₁′, headline L2=M₁′−M₁ intra-engine); BLK-1 mitigated
(600s time-limit + incumbent); BLK-2 dismissed (predicate-8 non-bug); critique-17 cheap fixes + doc drift done;
**F1 Slice C** (spot_cap C.5d both billing styles + generator spot_regime draw + persistence); **N3 `ReplayState`**
(clock + declarative-reconcile ledger [INT `capacity_ledger` + REAL `spot_capacity_ledger`] + `tendered_set`);
**2c-1** (`solve(pinned=…)` D-A11 hard pin). Governing: `arrival_only_replan_methodology.md` (§13 v4 + S38
Reading-B revisions to §4/D-A10/D-A11/D-A23), `backtest_methodology.md`, `precommitted_sla_deadline_proposal.md`.

**▶ Standing review agents next due S43** (calibration / interface-seam / backtest red-team; last run S36).
**Memory to add S39:** the Reading-B decomposition + L1=H₀−M₁′ anchor (a new `project_*` record).
